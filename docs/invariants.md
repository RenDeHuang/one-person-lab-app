...
- The App repo is a purpose-first Codex wrapper product authority. It owns how users enter research, grant, and presentation work; it does not own the runtime, domain, memory, artifact, or family production truth behind those entries.
...
- Standard updater assets update the desktop App only. Ordinary module install/update uses the stable package channel carried by Homebrew or App/CLI maintenance; GitHub repo or local checkout module sources are an explicit Developer Profile `source_channel` opt-in, not a normal-user default. Settings must express Developer Profile capabilities across `source_channel`, `workspace_trust`, `github_authority`, `agent_automation`, and `runtime_mutation_scope`, rather than a single Developer Mode switch. `opl-flow` is a Codex workflow/profile plugin, not a standard updater, WebUI image, or module package target.
- App release/user-path evidence is cohort-bound App evidence. It must not be used as stable/latest promotion, MAS/MAG/RCA domain readiness, or OPL family production readiness without the separate owner decisions and evidence gates.
