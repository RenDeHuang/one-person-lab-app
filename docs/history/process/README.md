...
| Current maintenance complexity boundary | [`docs/active/over-engineering-cleanup.md`](../../active/over-engineering-cleanup.md), contracts, source, tests, validators, and release evidence |
...
| Foreground alternative / archived proof / release / screenshot / guide evidence | 当前读法回到 OPL Studio foreground-alternative manifests、AGUI archived proof manifests、release artifacts、evidence manifests、generated guide manifests、CI logs、contracts、validators 和 tests。Hermes GUI candidate 的 registry、adapter、validator、命令和 runbook 已退役，旧字节只从 Git history 读取。Local smoke、VM proof、candidate smoke、archived proof smoke、screenshot proof、WebUI image-size、absolute paths、run ids 和 proof-by-proof logs 不作为当前 truth 保存。 |
...
| Over-engineering cleanup foldback | The completed 2026-07-10 implementation list, commit refs, pass totals, and command transcripts are historical provenance. The active path keeps only current risk boundaries and reopening gates; neither surface authorizes product retirement or upgrades source/tests into release proof. | [`docs/active/over-engineering-cleanup.md`](../../active/over-engineering-cleanup.md), [`2026-07-10-over-engineering-cleanup.md`](./2026-07-10-over-engineering-cleanup.md), contracts, source, tests, validators, release artifacts |
...
| GUI definition stack | GUI target shape, OPL delta, element audit, feature inventory, OPL Studio foreground-alternative plan, and AGUI archived-proof replay runbook remain separate owner docs. Shell implementation, active-shell adoption, and product acceptance require contracts, adapter selection, validation, and release gates. | GUI definition docs, App GUI/page-state/first-run contracts, active-shell validation |
...
| Settings Control Center audit compression | `settings-control-center-completion-audit.md` is a compact historical snapshot pointer and cannot prove current completion. Current authority stays with `contracts/app-settings-control-plane.json`, `docs/product/gui/settings-control-center.md`, the active five-axis ledger, Settings validators, exact-cohort visual/install evidence, and release records. The detailed 2026-06-30 evidence list, version/readback details, visual-manifest counts, workflow ids, and worktree notes remain historical process evidence in git history, release records, visual manifests, CI logs, and this compressed process index. | `contracts/app-settings-control-plane.json`, `docs/product/gui/settings-control-center.md`, `docs/active/app-ideal-state-gap-plan.md`, `docs/product/gui/shell-conformance-matrix.md`, Settings validators, visual manifests, release records |
| Hermes GUI candidate retirement | Hermes first-run and GUI adaptation once had executable candidate contracts, validators, package/smoke commands and runbooks. The current App has physically removed that chain; the focused no-resurrection test protects the deletion, while historical bytes remain available from Git history. | `tests/release/dual-gui-operating-policy.test.ts`, `docs/history/process/retired-surface-provenance.md`, Git history |
| GUI external-reference foldback | The feature inventory now keeps only current App-owned feature mappings for PilotDeck, Stitch, AG-UI and CopilotKit. Evaluated refs, source URLs, reference inventories, archived-proof replay commands and dated research/proof details stay with App contracts, the relevant AGUI runbook, manifests, artifacts, CI logs or history/provenance. | `docs/product/gui/feature-inventory.md`, `contracts/app-shell-candidates.json`, `docs/history/shell-candidates/agui-codex-candidate-verification.md` |
| Foreground alternative adoption and archived AGUI replay | Foreground alternative registry/adoption policy, design-reference policy, executable acceptance fields and candidate adapter selection are contract/validator-owned. The current foreground alternative is `opl-studio`; AGUI is the sole archived technical proof and explicit replay route. Hermes Desktop / `hermes-codex` is retired and absent from current registry, adapter, validator, command and runbook surfaces. Candidate smoke, manifests and runbook prose prove technical verification only. | `contracts/app-shell-candidates.json`, `contracts/shell-adapters/opl-studio.json`, `contracts/app-shell-adapter.json`, `scripts/validate-shell-candidates/*`, candidate manifests, validation scripts, `docs/product/gui/feature-inventory.md`, `docs/product/gui/opl-studio-plan.md`; AGUI replay provenance stays in `docs/history/shell-candidates/agui-codex-candidate-verification.md` and `contracts/shell-adapters/agui-codex.json` |
