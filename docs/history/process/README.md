...
| Retired GUI candidate deletion | AGUI and Hermes once had executable candidate contracts, validators, package/smoke commands and runbooks. The current App has physically removed both chains; the focused deletion test protects their absence, while historical bytes remain available from Git history. | `tests/release/dual-gui-operating-policy.test.ts`, `docs/history/process/retired-surface-provenance.md`, Git history |
