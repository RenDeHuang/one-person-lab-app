...
| Archived AGUI technical-proof replay | `docs/agui-codex-candidate-verification.md`, `contracts/app-shell-candidates.json`, `contracts/shell-adapters/agui-codex.json` |
...
| Candidate / archived proof / release / screenshot / guide evidence | 当前读法回到 Hermes candidate manifests、AGUI archived proof manifests、release artifacts、evidence manifests、generated guide manifests、CI logs、contracts、validators 和 tests。Local smoke、VM proof、candidate smoke、archived proof smoke、screenshot proof、WebUI image-size、absolute paths、run ids 和 proof-by-proof logs 不作为当前 truth 保存。 |
...
| Candidate shell adoption | Candidate registry/adoption policy, design-reference policy, executable acceptance fields and foreground alternative adapter selection are contract/validator-owned. The current foreground alternative is Hermes Desktop / `hermes-codex`; AGUI is archived technical proof and explicit replay only. Candidate smoke, manifests and runbook prose prove technical verification only. | `contracts/app-shell-candidates.json`, `contracts/shell-adapters/hermes-codex.json`, `contracts/app-shell-adapter.json`, `scripts/validate-shell-candidates/*`, Hermes candidate manifests, validation scripts, `docs/app-gui-feature-inventory.md`, `docs/opl-hermes-gui-adaptation-plan.md`; AGUI replay provenance stays in `docs/agui-codex-candidate-verification.md` and `contracts/shell-adapters/agui-codex.json` |
