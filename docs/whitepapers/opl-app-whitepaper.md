...
用户打开 OPL App，首先应该表达“我要完成什么”，而不是先选择仓库、provider、shell 或某个内部缩写。
...
这四个目的入口与 OPL Meta Agent（OMA）的“元智能体”入口共同构成默认首页起点，但不是 OPL App 的能力上限。专业能力以 **OPL Agent Packages** 进入工作台：用户可以安装、发现和管理更多合规 Package，并决定哪些快捷入口出现在首页。OMA 默认可见且仍可在“智能体”设置中关闭或调整顺序；首页保持简单，能力体系保持可扩展，两者不必互相牺牲。
...
**把下一轮接住。** 一周后回来，她看到的不是一段需要重新阅读的长对话，而是当前稿件、最近一次审阅、未关闭的问题和可以继续的动作。产品的价值不在于替她作最终学术判断，而在于让专业判断有材料、有上下文、有清楚的交接点。
