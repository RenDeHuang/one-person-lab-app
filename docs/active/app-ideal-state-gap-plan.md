...
| `packaged_gui_codex_path_evidence` | Home and ordinary conversation conformance are contract-backed and unit-tested. The packaged GUI assistant route smoke script and release evidence bundle require MAS/MAG/RCA selection, `@MAS/@MAG/@RCA` badges, hidden backend/model/permission selectors, and persisted `opl_assistant_route` receipts. A fresh local Full DMG run on 2026-05-29 produced `/tmp/opl-packaged-route-smoke-20260529063512-fail-summary/assistant-route-smoke-summary.json` with `status=failed`: the installed 26.5.28 UI exposed no `preset-pill-mas/mag/rca` entries and still showed an ordinary permission selector. Current-source packaged evidence now exists for both the `.app` bundle and DMG: `/tmp/opl-current-build-route-smoke-20260529065519/assistant-route-smoke-summary.json` and `/tmp/opl-current-dmg-route-smoke-20260529065705/assistant-route-smoke-summary.json` both report `status=passed` with MAS/MAG/RCA `type=acp`, `backend=codex`, `route_kind=builtin_capability`, `executor=codex_cli`, and `source=opl_app_home` receipts. This closes the current-source route receipt bug but does not retrofit the already-published 26.5.28 Full DMG evidence. | Published release evidence must use a DMG/App bundle that produces `artifacts/assistant-route-smoke-summary.json` and screenshots for MAS/MAG/RCA with passed Codex CLI route receipts; current-source evidence can be promoted only through a new packaged release cohort or explicit release evidence bundle. |
| `agui_codex_candidate_spike` | `contracts/app-shell-candidates.json` declares `agui-codex`; `contracts/shell-adapters/agui-codex.json` can explicitly select the linked `shells/agui-codex` external repo for a technical verification `.app` build. The target is the full Codex App-like OPL workbench inventory, not an AionUI modification list. CopilotKit is the visible UI/runtime layer for chat/sidebar/popup and agent runtime binding; AG-UI is the internal event/protocol layer and must not appear as ordinary user-facing product copy. On 2026-05-29 the external shell produced a usable chat-first CopilotKit/Codex app-server bundle: source UI smoke passed with `/tmp/opl-agui-codex-ui-smoke-final.png`, packaged `.app` UI smoke passed with `/tmp/opl-agui-codex-packaged-ui-smoke.png`, and both returned Codex reply `OK`. The App-root command surface is `npm run validate:shell-candidates`, `OPL_APP_SHELL_ADAPTER_CONTRACT=contracts/shell-adapters/agui-codex.json node --experimental-strip-types scripts/validate-active-shell.ts --quick`, and `OPL_APP_SHELL_ADAPTER_CONTRACT=contracts/shell-adapters/agui-codex.json npm run package`; candidate-shell checks are `npm run validate:candidate`, `npm run build:renderer`, source `npx electron . --ui-smoke-test`, and packaged `.app --ui-smoke-test`. The candidate now blocks on broader page-state/first-run/runtime mapping and replacement-gate promotion rather than blank-screen or dashboard-placeholder failure. | Candidate validation proves fixed Codex home, purpose entries, CopilotKit visible UI/runtime integration, AG-UI internal event mapping, OPL App state/action consumption, first-run/page-state mapping, App-wrapper launchable `.app` bundle output, source and packaged visible-pixel smoke against a real Codex app-server turn, no ordinary AG-UI/debug protocol copy, and release isolation before any default active-shell switch is considered. |
...
- Keep AG-UI/CopilotKit candidate docs aligned with `docs/app-gui-feature-inventory.md`: the target is a Codex App-like OPL workbench, CopilotKit is the visible UI/runtime layer, AG-UI is the internal event/protocol layer, and candidate verification must include explicit adapter selection, App-root packaging, source UI smoke, packaged UI smoke, and release isolation.
