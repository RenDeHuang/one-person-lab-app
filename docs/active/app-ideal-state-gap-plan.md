...
| Active shell upstream intake | `landed_verified` | `contracts/app-shell-adapter.json` records the current active shell upstream ref `82262a3f6cfe479bef7f7f464c3208e28db57ce8`; shell implementation history stays in `gaofeng21cn/opl-aion-shell`. |
...
| `legacy_build_and_release_workflow_retirement` | `.github/workflows/build-and-release.yml` still exists and can build from tag pushes, but the current Stable SSOT is the Desktop Release train: preflight, draft candidate, same-cohort gates, candidate record, and Promote. Release docs now classify the tag-push workflow as legacy/pending retirement rather than a valid standard-only Stable path. | Delete or disable the legacy tag-push release workflow after confirming no current release caller depends on it; keep any useful standard-build material only in the Desktop Release train or reusable build workflow, with release-boundary tests updated to fail closed on the retired entry. |
...
| `runtime_page_operator_evidence` | Runtime page consumes OPL App/operator drilldown, refs-only safe action routes, and Stage Artifact Kernel workbench refs. The active project line projection and artifact-native drilldown projection are contract-backed in `contracts/app-runtime-bridge.json`, `contracts/app-page-state-matrix.json`, and `contracts/app-gui-product-contract.json`; `scripts/validate-active-shell.ts --quick`, `tests/release/app-runtime-bridge-boundary.test.ts`, and `tests/release/app-release-boundary.test.ts` fail closed if queued/escalated project lines lose `status`, `active_run_id`, `next_visible_step`, are presented as active worker runs, or if artifact drilldown grows artifact body/domain readiness authority. | Page-state tests and GUI smoke show summary-first read model, active project line separation, artifact-native refs-only drilldown, lazy full detail, dry-run/execute controls, receipt/count refresh, and authority-boundary fields. |
...
- Retire the legacy tag-push **Build and Release** workflow as a separate workflow surface once no active release caller depends on it; ordinary Stable releases must stay on the Desktop Release train and candidate-record promotion path.
