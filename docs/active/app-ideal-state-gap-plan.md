...
| Runtime owner-action default | `landed_contract_and_test_backed` | Runtime page default attention is user-task-status first: running, active, queued, attention, and each task's next visible step. Active project lines come from App-consumed OPL refs and must preserve `status`, `active_run_id`, and `next_visible_step` without claiming active worker run, domain readiness, or paper quality. Full ledger detail and provider diagnostics are on-demand diagnostic, audit, or release evidence only. |
...
| Active shell upstream intake | `landed_verified` | `contracts/app-shell-adapter.json` records the current active shell upstream ref `1efb046736d1d61384508eb0ebce7eabbe824ce2`; shell implementation history stays in `gaofeng21cn/opl-aion-shell`. |
