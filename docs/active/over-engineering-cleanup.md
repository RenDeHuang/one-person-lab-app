...
| --- | --- | --- | --- |
| 1 - high (`cleanup-package-manager`) | Native Package lifecycle migration and old authority removal | **In progress, first family canonical:** fresh presence/App-state projection and installed-only invocation are canonical. Ordinary invocation no longer refreshes owner catalogs or creates `offline_lkg`/`recovered_last_known_good`; explicit install/update/remove/repair still retain lock, payload/materialization, receipt, rollback and mutex compatibility. | Follow the sole migration SSOT. Do not create a smaller custom Package manager as the destination; delete each remaining legacy family only after its replacement is canonical, live acceptance passes and retained consumers are zero. |
...
| 6 - medium (`legacy-cleanup-carrier-cadence`) | Carrier/channel/cadence simplification | Source/contract cleanup is canonical: App Standard and WebUI are independently qualified carriers, legacy desktop aliases are absent, and Framework Daily is reconciliation-only. Live Stable/Latest, first Nightly publication/follower readback, WebUI `:stable`, and single-Package terminal proofs remain pending. | Keep Stable/Preview as the quality axis, Latest as an independent updater pointer, Docker/WebUI/Homebrew as carriers, Full as an exact snapshot, and Framework Daily as reconciliation. Nightly is the automated Standard Preview: its schedule defaults to no Latest mutation and has no Full/WebUI/heavy-VM authority. A separate protected single-use expected-current CAS may temporarily select an exact published Preview without changing quality; the next qualified Stable reclaims Latest by default. Canary is validation-only. Do not call any carrier complete until its exact public readback exists. |
