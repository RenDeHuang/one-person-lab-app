...
| 8 - low (`A4`) | Implementation-literal acceptance pruning | Candidate; behavior gates remain authoritative and no bulk test deletion is authorized. | Replace source strings, function names, CSS literals, and exact callsite counts as primary acceptance with behavior, accessibility, owner readback, and installed-pixel evidence. Retain only narrow structural smoke checks that detect a real unsupported integration boundary. |
| 9 - low (`A5`) | Hand-copied upstream/version truth pruning | Candidate; exact source and release locks remain required. | Generate the App-facing projection from the Shell source lock, managed manifest, Framework Release Set, and release receipt. Remove stale intake refs or hand-maintained AionCore/Framework constants only after exact owner refs remain visible and fail closed. |
