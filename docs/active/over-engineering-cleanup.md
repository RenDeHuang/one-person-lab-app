...
| 6 - medium (`legacy-cleanup-carrier-cadence`) | Carrier/channel/cadence simplification | Source/contract cleanup is canonical: App Standard and WebUI are independently qualified carriers, legacy desktop aliases are absent, and Framework Daily is reconciliation-only. Live Stable/Latest, first Nightly publication/follower readback, WebUI `:stable`, and single-Package terminal proofs remain pending. | Keep Stable as release policy, Docker/WebUI/Homebrew as carriers, Full as an exact snapshot, and Framework Daily as reconciliation. Nightly is a separate automatic Standard prerelease with no Latest or heavy-VM authority; Canary is validation-only. Do not call any carrier complete until its exact public readback exists. |
