...
Framework resolves compatible candidates and writes the exact installed lock;
App/Shell render projections and delegate actions. A Full/Release Set is an
exact reproducible snapshot, not a seven-package publication transaction.
Developer, external, manual, and offline sources remain candidate adapters and
must not define ordinary Stable currentness.
...
| 2 - high (`A6`) | Fixed starter-profile and package lock-step release removal | Planned: the current starter profile and some release metadata still enumerate a fixed seven-package composition. | Keep a starter profile only as a replaceable default. Each package publishes independently with SemVer, immutable digest, dependency closure, and ABI ranges. Release Set remains a Full/offline or qualification snapshot. Do not remove current starter UX until a Framework index/resolver and exact installed-lock readback work for a non-starter package. |
| 3 - high (`A7`) | Package source/currentness consolidation | Planned: Developer checkout, external registry, OCI, and offline sources still need one explicit resolver boundary. | Preserve all source adapters, but only Framework resolver decides target currentness and the installed lock decides local truth. A source mismatch must be diagnostic, never a second update authority or an automatic managed update. |
...
| 6 - medium (`A8`) | Carrier/channel/cadence simplification | Planned: Stable, Docker/WebUI, Full, Nightly, and Daily still have overlapping release/promotion narratives. | Keep Stable as release policy, Docker/WebUI/Homebrew as carriers, Full as an exact snapshot, Nightly as optional canary, and Daily as cadence/index reconciliation. Remove redundant promotion workflows only after each retained carrier has an owner and terminal readback. |
