...
- A non-OPL active provider remains active. OPL may register or refresh an inactive OPL Gateway provider entry without replacing the user's root provider, model, or reasoning values. New entries use the OPL-reserved `oplgateway` ID; an existing `oplgateway` entry is always refreshed in place and suffix variants are never created. Existing recognized legacy Gateway aliases such as `gflab` and `gflabtoken` remain in place and are not renamed.
