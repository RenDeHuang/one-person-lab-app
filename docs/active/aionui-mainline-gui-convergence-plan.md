...
`285deb60b0cd0b5ffd1bae77bde526236a12aa17` 已补齐
...
绿色 locale-aware account avatar 与 `circle_question` help icon。历史 macOS
...
| Shell current source cohort | `285deb60b0cd0b5ffd1bae77bde526236a12aa17`（基于 main `6639744c...`） | 叠加managed Worktree snapshot/cleanup/restore（含ignored user files）、Review focus expected-turn steer与profile-driven avatar/help。Model-facing host tool、line-level comments和cross-host handoff仍为typed protocol blocker；尚无匹配package/pixels。 |
...
| Current source gate boundary | Shell `285deb60b...`：full active-shell `302 files pass / 1 skip`、`2415 pass / 9 skip`；TypeScript、1548-file format、i18n与lint `0 errors / 888 warnings`通过；avatar/help owner focused logic `28 pass`、DOM `5 pass`。App release-boundary `351 pass / 2 platform skip` | Source gate计数均绑定final exact source或该exact包含的focused owner handoff。没有运行package/build/E2E/install，不提升package/pixels/install状态。 |
| Historical package-bound visual evidence | `docs/product/gui/evidence/aionui-41301-parity-20260714/manifest.json`绑定Shell `b2c05a1c...`、9场景、0 unmatched anchors、0 failed layout checks、0 coverage gaps | macOS arm64 directory-only package、packaged runtime staging与codesign通过；`app.asar` SHA-256=`726200362ed6038211dfb610b7639cb7fe395df54b92bade752c9b8f5a538823`；未安装，`release_ready=false`，不能改绑到`285deb60b...`。 |
...
2. Shell `285deb60b...` 已闭合同主机handoff、Home worktree、canonical user thread coordination、
   ...
   Access布局；本轮新增durable Worktree snapshot/cleanup/restore与Review focus expected-turn steer。
   Model host tool、Review line-level comments与cross-host handoff仍为protocol-owner blocker。
3. `285deb60b...` 的full active-shell、TypeScript、format、i18n、lint 0 errors、avatar/help focused
   gates，以及App release-boundary已闭合；
...
| Shell GUI behavior 与 OPL 非降级边界一致 | `current_source_gates_verified_protocol_owner_gaps` | `285deb60b...` 保留OPL已用功能、Gateway/UI与Runtime V2，并新增durable Worktree lifecycle、Review focus delivery与avatar/help policy；source gates已闭合，新package/pixels仍待闭合。 |
