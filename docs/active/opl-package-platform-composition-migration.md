...
| Role-neutral App contributions v1 | `canonical_source`：App contract/broker、Framework directory/App-state/action bridge、AionUI navigation/standard-view renderer 和 Relay carrier data/action route 已分别进入 canonical main；投影不按 `package_role` 或 executor 过滤，invalid Package 保持局部 fail-closed。 | Source 吸收不等于生效证明；仍须分别完成 Pixel、Install、Apple Mail review path 和 Release proof。 |
