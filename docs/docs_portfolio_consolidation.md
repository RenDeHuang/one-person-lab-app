...
The root `docs/` layer is an index and governance layer. It should keep only the
core current docs, `docs/active/app-ideal-state-gap-plan.md`, and
`docs/docs_portfolio_consolidation.md`. Specific product, delivery, testing,
public, and history material belongs in the target taxonomy below instead of
growing more root-level topic files.

The target App taxonomy is:

- `docs/public/`: user-readable public entry and generated/shareable guide
  bundles.
- `docs/product/`: App/workbench/product shell design and GUI support.
- `docs/delivery/`: release, artifact/package/export lifecycle, user-guide
  generation source, screenshot provenance, and verification support.
- `docs/testing/`: validation command entry and test/evidence classification.
- `docs/history/`: retired routes, candidate verification provenance, stale
  surface no-resurrection notes, and historical process records.

`docs/product/` and `docs/delivery/` may be absent until their parallel IA lanes
land. While that absorption is pending, existing `docs/release/`,
`docs/user-guides/`, and `docs/screenshots/` remain legacy delivery-owned
directories, and root-level GUI/candidate files remain readable but should not
grow new scope.
...
| `docs/product/` | Product shell docs; target active taxonomy | App/workbench/product shell design, GUI support, and product-facing reference docs after the product lane lands | Product acceptance stays in App contracts, page-state matrices, active-shell validation, source, and tests |
| `docs/delivery/` | Delivery docs; target active taxonomy | Release, artifact/package/export lifecycle, user-guide generation source, screenshot provenance, and verification support after the delivery lane lands | Release/delivery truth stays in assets, updater metadata, evidence manifests, CI/logs, workflows, validators, and release-boundary tests |
...
| `docs/release/` | Legacy delivery directory pending delivery-lane absorption | Release operator map and release policy while final delivery paths are unsettled | Same release boundary as `docs/delivery/`; do not make release-ready claims from prose |
| `docs/user-guides/`, `docs/screenshots/` | Legacy delivery directories pending delivery-lane absorption | Guide source, screenshot provenance, visual tutorial notes, and generated verification records | Source/maintenance docs; public reading artifacts are generated into `docs/public/` |
| Root GUI/candidate files | Product/history material pending product/history absorption | GUI target docs, Hermes foreground alternative notes, and archived AGUI replay runbook | Acceptance stays in contracts, validators, manifests, shell artifacts, release artifacts, tests, workflows, or CI logs |
...
- Root `docs/` should not accumulate new topic files. Add or move material to
  `public/`, `product/`, `delivery/`, `testing/`, or `history/` once the owning
  lane/path exists.
...
- `docs/delivery/` owns release, artifact/package/export lifecycle, user-guide
  generation source, screenshot provenance, and verification support. Release
  claims must fold back to artifacts, updater metadata, evidence manifests,
  workflows, validators, release-boundary tests, CI logs, or release records.
...
| --- | --- |
| Durable App product or release rule | Core docs, active gap plan, App contracts, source, tests, workflows, release validation docs, or `docs/product/` / `docs/delivery/` once those paths land |
...
| Release proof, remote checks, VM smoke, packaged route receipts | Release artifacts, evidence manifests, CI logs, release records, workflows, or release history/provenance |
| Foreground alternative technical smoke, adoption gate, or replacement decision | `contracts/app-shell-candidates.json`, `contracts/shell-adapters/hermes-codex.json`, `scripts/validate-shell-candidates/*`, Hermes candidate manifests, shell artifacts, focused tests, candidate history/provenance, and `docs/product/` once landed; archived AGUI replay evidence stays under AGUI provenance only when AGUI is explicitly requested |
| GUI definition / interaction target | `docs/product/` once landed; until then the root GUI files plus App GUI/page-state/first-run contracts and active-shell validation |
| User guide generation and screenshot provenance | `docs/delivery/` once landed; until then `docs/user-guides/`, `docs/screenshots/`, and generated artifacts in `docs/public/macos-app-install/` |
