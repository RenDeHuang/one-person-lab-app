...
| `docs/agui-codex-candidate-verification.md` | Archived AG-UI/CopilotKit technical-proof replay runbook for explicit adapter command order and false-authority boundaries | Executable replay acceptance stays in `contracts/app-shell-candidates.json`, `contracts/shell-adapters/agui-codex.json`, `scripts/validate-shell-candidates/*`, candidate manifests, shell artifacts, CI logs, release evidence, or history |
