...
| Foreground alternative technical smoke, adoption gate, or replacement decision | `contracts/app-shell-candidates.json`, `contracts/shell-adapters/opl-studio.json`, `scripts/validate-shell-candidates/*`, Studio candidate manifests, shell artifacts, focused tests, and candidate history/provenance; archived AGUI replay evidence stays under `docs/history/shell-candidates/` only when AGUI is explicitly requested |
