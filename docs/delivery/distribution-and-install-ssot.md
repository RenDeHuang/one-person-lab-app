...
| Nightly | Implemented；公开 currentness 由 fresh readback 决定 | 自动 Standard DMG/ZIP/updater prerelease + Nightly Cask follower | 每日 schedule 或显式确认的 owner dispatch 运行同一 protected path，均不改 Latest；独立 protected pointer operation 可临时选择 exact Nightly；不含 Full/WebUI、不复用 Stable mutex；抽样 VM 非阻塞 |
