...
| Nightly | Derived Automated Preview kind, independent of density; production defaults to the daily schedule, while a user-explicit `development_validation` may use protected `workflow_dispatch` to execute the same Standard-density immutable publication path with `include_full=false` and `make_latest=false`; frozen request/receipt identity distinguishes both modes before isolated Homebrew and sampled-VM followers | A separate protected single-use expected-current CAS may temporarily select an exact published Nightly without changing quality; current publication and follower state always require fresh readback |
