...
| Nightly | Derived Automated Preview kind, independent of density; the daily schedule or explicitly confirmed owner dispatch publishes Standard-density immutable GitHub prerelease assets with `include_full=false` and `make_latest=false`, then starts isolated Homebrew and sampled-VM followers | A separate protected single-use expected-current CAS may temporarily select an exact published Nightly without changing quality; current publication and follower state always require fresh readback |
