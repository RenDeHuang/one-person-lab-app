...
5. 确认 Docker Desktop 使用 **Linux containers**。如果菜单显示“Switch to Linux containers”，请先执行切换。

打开 PowerShell，确认以下命令可以正常运行：
...
Windows 要求重启时，先重启并打开 Docker Desktop，再重新运行普通一键安装命令。
...
$p = Join-Path $env:TEMP 'install-docker-webui.ps1'
iwr -UseBasicParsing "$root/scripts/install-docker-webui.ps1" -OutFile $p
powershell -ExecutionPolicy Bypass -File $p -Yes
```
...
Windows 日常使用前先启动 Docker Desktop；Linux 先确认 Docker Engine 正常。浏览器继续访问 <http://localhost:3000/>。

更新 WebUI 时重新运行同一条一键安装命令即可。安装器会拉取镜像并重建容器，同时保留：
...
打开 Docker Desktop 并等待 Running；在 PowerShell 中重新执行 `wsl --status`、`docker version` 和 `docker compose version`。如果依赖确实缺失，可使用前文的管理员依赖安装命令。
