...
安装器会拉取 One Person Lab WebUI 镜像、创建 `compose.yaml` 和两个持久目录，然后启动 WebUI。Windows 推荐命令还会为当前用户注册 **One Person Lab WebUI Latest Update** 计划任务，每天 03:00 检查 `latest`；电脑关机或用户未登录时不会后台强行运行，下一次登录后由 Windows 在可用时补跑。默认映射如下：
