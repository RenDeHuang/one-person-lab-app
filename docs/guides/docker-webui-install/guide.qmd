...
适用对象：Linux、Windows、本机 Docker 用户和服务器管理员；默认没有 Docker 经验的用户走本机一键安装路径。

新手主路径是先让 Docker 正常运行，再运行 One Person Lab 提供的一键安装器，最后在浏览器里完成首次配置。手动 Docker 命令只作为排障或高级部署路径保留。

本机一键安装默认只绑定 `127.0.0.1:3000`，浏览器自动建立本机 WebUI 会话，不要求输入 WebUI 用户名和密码。云端或公网部署不是这个默认路径：只要显式设置 cloud/password 模式，或传入 WebUI 密码、OPL Gateway API Key 等外部 secret，就必须启用 WebUI 登录密码，默认用户名是 `opl`，并通过 Docker secrets 或 `*_FILE` 传入。
...
## 6. 云端部署：使用密码、secrets 和 TLS

当 WebUI 部署在云服务器、VPS、办公室服务器或任何可能被非本机浏览器访问的位置时，不要使用默认免密码路径。云端部署使用仓库内的模板包：

```bash
sh ./scripts/install-docker-webui.sh --cloud-template \
  --cloud-template-dir "$HOME/OnePersonLab/cloud"
cd "$HOME/OnePersonLab/cloud"
cp .env.example .env
```

编辑 `.env`。这里放域名、邮箱、镜像和用户名；不要把真实 WebUI 密码或 API Key 直接写进 `.env`：
...
创建必需的 WebUI 登录密码文件。这个密码用于浏览器登录 WebUI，不是模型/API 访问密钥：

```bash
mkdir -p secrets data projects
printf '%s\n' '<强 WebUI 登录密码>' > secrets/webui_password
chmod 600 secrets/webui_password
```

启动带 Caddy HTTPS 反向代理的 WebUI：

```bash
docker compose -f compose.yaml up -d
```

如果管理员要提前配置 OPL Gateway API Key，再创建可选 secret，并叠加 overlay：
...
| --- | --- | --- | --- | --- |
| WebUI 登录密码 | `secrets/webui_password` | `OPL_WEBUI_PASSWORD_FILE=./secrets/webui_password` | `/run/secrets/opl_webui_password` | 浏览器登录 WebUI，云端部署必需 |
...
```text
浏览器地址：https://<OPL_WEBUI_DOMAIN>/
用户名：.env 里的 OPL_WEBUI_USERNAME，默认 opl
密码：secrets/webui_password 文件里的内容
```

云端规则：

- WebUI 登录密码和 OPL Gateway API Key 是两件事；API Key 不能替代登录密码。
- `secrets/webui_password` 是必需文件；`secrets/gateway_api_key` 只有需要启动时预配置 Gateway 才创建。
- 只传 `OPL_GATEWAY_API_KEY_FILE` / `OPL_GATEWAY_API_KEY` 而没有 `OPL_WEBUI_PASSWORD_FILE` / `OPL_WEBUI_PASSWORD` 时，容器必须拒绝启动。
- 推荐 Docker secrets 和 `*_FILE`；明文环境变量只作为高级兜底。
- 公开访问只走 Caddy/反向代理的 HTTPS 地址，不把 WebUI 容器直接裸露到公网。
- 云端模式禁用浏览器自动登录；用户访问域名时输入用户名 `opl` 或 `.env` 中的 `OPL_WEBUI_USERNAME`，以及 secret 文件里的 WebUI 密码。
