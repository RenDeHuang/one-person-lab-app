...
`One-Person-Lab-Full-<版本>-mac-arm64.dmg`。Standard 会先成为 Latest，Full 随后追加到同一
Release/tag；刚发布时如果暂未看到 Full，请等待追加发布完成并刷新 Assets，不要改下 ZIP、Source code
或其他元数据文件。
...
进入主界面后，可以直接描述目标，也可以选择当前安装的 Package 动态提供的工作入口。8.22 界面中的
当前入口包括“开展科研工作”“开展基金申请工作”“制作视觉交付物”和“构建与迭代智能体”；后续安装、
启用或移除 Package 时，首页会随 Framework 的实时投影变化，不维护固定入口清单。本教程以科研任务为例。
...
- **下载页面里文件很多，应该选哪个？** 首次安装选择 `One-Person-Lab-Full-<版本>-mac-arm64.dmg`。刚发版时 Full 可能正在向同一 Release 追加，请等待它出现；不要下载 ZIP、blockmap、YML、JSON 或 Source code。
