...
| Home/Guid | Codex model/status | 表示固定 Codex CLI executor、默认 5.6 Sol / ultra 和 Codex App 顺序的可见模型选择策略；推理强度在模型菜单内配置。 | Header 或 composer 低权重状态。 | 当前 contracts 允许 Home/conversation 显示 model selector/status，但仍禁止 backend/provider/permission selector。 |
