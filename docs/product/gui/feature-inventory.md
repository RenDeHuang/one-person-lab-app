...
| `B0-02` | Home/New task、session/thread 目录与历史管理 | 新建、恢复、搜索、pin、rename、archive/restore 是日常入口。 | Ordinary rail 只读取 OPL conversation store；canonical/global directory 不能被枚举、合并或 fallback 注入。Session 是身份单位；Project affinity 为零或一，并只来自 OPL 权威 metadata。 |
...
| --- | --- | --- |
| Session-first workspace-aware App frame | OPL conversation 是普通产品面的身份单位；Project affinity 最多一个，workspace affinity/classification 为 `explicit | temporary | none | legacy_unknown`，且只接受 OPL 持久化的权威 metadata。project/workspace/directory 提供新 session 初始 cwd、projectless 一次性 adoption 和 affinity rail 分组，不拥有 session、context 或 artifact，也不构成授权域。命令或 turn 的实际 `pwd`、canonical cwd、显式输入、writable roots、Git origin、标题或时间均不得推断或改写 affinity。 | GUI contract、product profile、OPL conversation store、Codex permission/approval/sandbox。 |
| Directory/conversation navigation | 宽桌面 rail 默认展开，窄窗口变 drawer；ordinary rail 只读取 OPL conversation store，不能调用、枚举、合并或通过 fallback 注入 Codex App Server canonical/global directory。Canonical thread ID 只作为已知 OPL conversation 的显式生命周期引用，不是 ordinary history 数据源。Legacy conversation 缺少权威 affinity metadata 时保持可见并标记 `legacy_unknown`/未归类；目录组不提供组级删除或 session 级联删除。 | GUI contract、page-state matrix、OPL conversation store。 |
...
| Resume conversation | 先从 OPL conversation store 找回 recent conversation，保留 transcript/turn history/title/task state 与权威 Project-affinity metadata；仅在该已知 OPL conversation 已持久化 canonical thread ID 且用户显式恢复时，才调用 App Server resume/read。 | OPL conversation store；App Server 仅拥有被显式调用的 canonical lifecycle。 |
...
| User-triggered thread operations | 仅针对已知 OPL conversation，从现有 actions 显式 open/read/resume/fork/archive/restore/rename/delete canonical thread；adapter 不执行 global list/start，不向 ordinary rail 合并或 fallback 注入 canonical directory。普通对话继续走 AionUI ACP，不增加独立 coordination 页面或模型工具。 | 一个 Codex App Server explicit-lifecycle adapter；OPL conversation store 持有 ordinary directory/history authority。 |
| Session Project affinity / working directory | Home/new-session composer 上方独立 context bar 只设置新任务初始 cwd，working directory 不进入 `+` palette。仅 OPL 权威 metadata 明确为 temporary/projectless 的 conversation 可由用户从 rail 一次性归入一个目录，成功后原子持久化 `workspace_affinity=explicit`、`is_temporary_workspace=false` 与所选 workspace，再移动 row。失败保持原 metadata 且对话可继续；legacy 缺 metadata 时保持可见并标记 `legacy_unknown`/未归类。已有 explicit affinity 的 session 不提供 A→B 改绑、无真实 adapter 的 Local/Worktree 切换或 managed Worktree。Turn cwd、shell `pwd`、canonical cwd、Git origin、标题、时间、显式文件/目录输入与 writable roots 均不得用于猜测 affinity。 | OPL conversation store 持有 Project/workspace affinity authority。Canonical thread ID 仅是显式 lifecycle ref，不是 affinity 或 ordinary history authority。 |
