...
- Current GUI parity integration：`opl-aion-shell@fae4f694b1ab3eb615b7f527b792adfa6b3165e1`。
...
- Pre-parity source cohort：Shell `a0ce713b65801fd9ca7f46ad168c977c75a187de` full suite
  ...
  visible coordination/unarchive、首结果 replay、Local/Worktree/handoff 或 Review。
...
- Historical packaged visual evidence：[`evidence/aionui-41301/manifest.json`](evidence/aionui-41301/manifest.json)
  ...
  zh-CN/en-US Home/conversation 状态；旧 manifest 不修改 SHA。current source `fae4f694...` pixels
...
| P0 | Project hierarchy | Project 是 thread 的默认 cwd、分组与 context hint；rail canonical history/actions 来自 App Server。 | `fae4f694...` 已投影 App Server thread directory/actions；pin 保持 UI metadata，local reset 不冒充 history reset。 | `source_implemented`, `pixel_unverified` | Shell DB 只留 draft/preferences/cache；保持 canonical identity 与 rebuild tests。 |
...
| P1 | Artifact preview adapter | 用户显式合法绝对本地路径或 workspace-scoped project ref 薄接现有 Preview；traversal、非法 scheme、自动静默读取返回明确失败。 | `fae4f694...` 已复用既有 renderer/store，覆盖 projectless file access、absolute-path 与 workspace-ref 分流。 | `source_implemented`, `pixel_unverified` | 保持 ref-only 与 Codex permission authority；current renderer pixels另行。 |
| P1 | Cross-thread coordination | Rail 有可见键盘入口；App Server list/read/resume/fork/archive/unarchive/start/steer；同 key 返回首个 receipt/result、`ok=true`；cross-host 当前明确 unavailable。 | `fae4f694...` 已闭合 visible entry、App Server lifecycle、flexible advisory、running steer、首结果 replay 与无额外 confirmation；cross-host 返回 unsupported。 | `source_implemented`, `pixel_unverified` | 只补 current pixels/package；不得把 unsupported 包装成 handoff success。 |
| P1 | Local / Worktree lifecycle | Home 新任务选择 Local/Worktree 与 starting branch；既有同主机 `not_loaded`/`idle` task 在 Environment 双向 handoff，真实 cwd 先于 projection 更新。 | `fae4f694...` 已实现 same-host 子集：复用 `gitWorkspace` 与 `thread/settings/update`，使用 `opl_workspace_handoff.v1`，projection failure best-effort 回滚 cwd，worktree 默认保留复用。 | `source_partial`, `pixel_unverified` | Same-host source已闭合；snapshot/restore、cleanup UI deferred，cross-host unsupported，完整 lifecycle 不得宣称完成。 |
| P1 | Review pane | 复用 Files/Changes；四类 target、inline/detached、PR context、stage/commit/push；缺失能力显示 unavailable。 | `fae4f694...` 已实现四类 `review/start` target、inline/detached delivery、PR context、stage/commit/push 和 `gh` unavailable；尚无 `last_turn` section 与 line-level inline comment UI/投递。 | `source_partial`, `pixel_unverified` | 保持 thin adapter；补 `last_turn` 和 line-level inline comments，不恢复 legacy equal-weight Review tab。 |
...
| Project task 与 projectless conversation | `aligned_contract` | `source_implemented` | `pixel_unverified` | `current_contract_deviation` | `source_partial` | `pixel_unverified` | `fae4f694...` 保持无 workspace text/attachment、任意 file/directory picker、paste/drop 与 `/open` 可用，访问只服从 Codex permission。 |
...
| 一个项目对应 N 个 App Server threads | `aligned_contract` | `source_implemented` | `pixel_unverified` | `candidate_target` | `source_partial` | `pixel_verified` | `fae4f694...` 以 `thread/list/read/resume` 为 canonical directory；本地 history 只作可重建 cache。 |
| 对话 search/pin/rename/archive/restore/delete/reset 与独立 Archived | `aligned_contract` | `source_implemented` | `pixel_unverified` | `current_contract_deviation` | `source_missing` | `pixel_unverified` | `fae4f694...` 将 rename/archive/restore/delete 映射 App Server；pin仅 UI metadata，local reset不冒充 history reset。 |
...
| Environment popover 与 workspace surfaces 分离 | `aligned_contract` | `source_implemented` | `pixel_unverified` | `current_contract_deviation` | `source_partial` | `pixel_unverified` | `fae4f694...` 的 Environment 使用 live Git inspection并承载同主机空闲 task locality handoff；Files/Preview 只在用户或任务需要时出现。 |
...
| Artifact Markdown/PDF/Mermaid/Code/KaTeX preview | `aligned_contract` | `source_implemented` | `pixel_unverified` | `candidate_target` | `source_partial` | `pixel_unverified` | `fae4f694...` 已覆盖用户显式绝对本地路径、workspace project ref与拒绝 traversal/非法 scheme/静默读取；renderer pixels仍独立。 |
| Local/Worktree new task 与同主机 idle handoff | `aligned_contract` | `source_partial` | `pixel_unverified` | `candidate_target` | `source_not_assessed` | `pixel_unverified` | `fae4f694...` 覆盖 same-host managed create/reuse、`thread/settings/update`、metadata 与 rollback；完整 lifecycle仍缺 snapshot/cleanup，cross-host unsupported。 |
| Review 复用 Files/Changes diff surface | `aligned_contract` | `source_partial` | `pixel_unverified` | `candidate_target` | `source_not_assessed` | `pixel_unverified` | `fae4f694...` 覆盖四 targets、inline/detached、PR context、stage/commit/push 与 `gh` unavailable；缺 `last_turn` 和 line-level inline comments。 |
...
- **Core pixels：** 8 场景只覆盖历史 `0ebc1fdd...` 的 Home、conversation、composer/model、
  Environment、Files、mobile action sheet 与 mobile Preview；current source `fae4f694...` 尚未重拍，
...
- **Cross-thread coordination：** Shell `fae4f694...` 已实现 visible entry、unarchive、首结果
  idempotency replay与same-host routing；cross-host当前返回 unsupported，只有 future remote evidence
  完成后才能提升。
- **Artifact ref adapter：** `fae4f694...` 已覆盖用户显式任意绝对本地路径、workspace ref、双 scope
  policy和非法输入拒绝；current renderer pixels/package仍需独立证据。
- **Codex locality / Review：** `fae4f694...` 已实现same-host Home worktree与idle task handoff，但
