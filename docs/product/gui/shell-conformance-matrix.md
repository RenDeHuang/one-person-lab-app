...
| `R1-06` | OPL bundle、update、deep link、feedback/support。安装、唤起、更新和求助必须是同一产品。 | `aligned_contract` | `L1 profile + L2 bridge/adapter` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | AionUI 已有 `aionui://` parser/registration、冷启动 pending queue、second-instance 与 macOS `open-url`。剩余是迁移到 branded `opl://`、只保留必要 legacy compatibility、增加 action allowlist/schema，并禁止 raw query 或 base64 payload 携带 `api_key`/secret；补 cold/warm/second-instance/invalid/security tests。Native 另补正式 updater/deep-link/support。 |
...
| X0-01 | Runtime cockpit | 条件保留的跨项目 Work Item projection / owner route；不是 B0/R1/U1 核心。 | Runtime V2 与入口已有 source，可保留但不要求 Native phase-1 重做；当前 Pixel 不能由历史 cohort 回填。 | `current_contract_deviation` | `source_implemented` | `pixel_unverified` | `install_unverified` | `release_unverified` | 现有 Runtime contract、page-state、design-system/release validators 仍把它当 P0/硬门；后续 machine cleanup 应移出默认 release gate，validator pass 只证明现有合同/source 一致。 |
...
|   Rail 顶部 New task/Archived；Runtime 仅为条件保留的 X0-01 入口，底部 account/help/Settings | `current_contract_deviation` | `source_implemented` | `pixel_unverified` | `not_claimed` | `source_not_assessed` | `pixel_unverified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | AionUI 当前保留 Runtime，产品分类为 `retained_x0_route`；现有 contract/validator 硬门是独立 maintenance debt。Native phase-1 不以缺少 Runtime 为 parity gap。   |
