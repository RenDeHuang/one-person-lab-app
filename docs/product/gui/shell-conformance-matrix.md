...
- Final GUI parity integration candidate：
  `opl-aion-shell@dd67a50c1eca3548b47cbb9144a1a08a403e72f3`，基于 local main
  `c812c6715494be33d48a447ddec377a550e5e8e8`，尚未吸收 main。
  Fresh source readback覆盖同主机 Local/Worktree handoff、Home managed worktree、Review已实现子集、可见
  thread coordination、projectless file access、Runtime v2 与 Environment Git inspection；该 SHA
  只提升 source status，不提供 current pixels、package、installed-path 或 cross-host evidence。
...
- Pre-parity source cohort：Shell `a0ce713b65801fd9ca7f46ad168c977c75a187de` full suite
  ...
  visible coordination/unarchive、首结果 replay、Local/Worktree/handoff、Runtime cockpit 或 Review。
...
- Historical packaged visual evidence：[`evidence/aionui-41301/manifest.json`](evidence/aionui-41301/manifest.json)
  ...
  zh-CN/en-US Home/conversation 状态；旧 manifest 不修改 SHA。candidate `dd67a50c1...` pixels
...
| P0 | Project hierarchy | Project 是 thread 的默认 cwd、分组与 context hint；rail canonical history/actions 来自 App Server。 | `dd67a50c1...` candidate已投影 App Server canonical thread directory/actions；pin 保持 UI metadata，local reset 不冒充 history reset。 | `source_implemented`, `pixel_unverified` | Shell DB 只留 draft/preferences/cache；保持 canonical identity 与 rebuild tests。 |
| P0 | Home / New task | 与 conversation 共用 chat canvas/composer，不是 dashboard；新任务可选 Local/Worktree 与 starting branch。 | Composer-first Home、全部用户可见 starters、stable order、responsive wrap、package readiness 以及既有 `gitWorkspace` managed worktree create/reuse 已实现。 | `source_implemented`, `pixel_unverified` | 防止恢复 launcher/card wall、静默四项截断、Worktree 失败 silent Local fallback 或 unavailable package 继续 launch。 |
...
| P0 | Environment details | 右上 anchored floating summary，默认关闭；承载既有同主机 task locality handoff 与 Git context。 | `dd67a50c1...` candidate仅渲染真实 workspace/locality/branch/changes/refs，使用 live Git inspection，并为 `not_loaded`/`idle` task 提供 Local↔Worktree；与 Files/Preview 分离。 | `source_implemented`, `pixel_unverified` | `running`/`archived`/`system_error` 保持 unavailable；无真实数据不显示，不 silent fallback。 |
...
| P1 | Artifact preview adapter | 用户显式合法绝对本地路径或 workspace-scoped project ref 薄接现有 Preview；traversal、非法 scheme、自动静默读取返回明确失败。 | `dd67a50c1...` candidate已复用既有 renderer/store，覆盖 projectless file access、absolute-path 与 workspace-ref 分流。 | `source_implemented`, `pixel_unverified` | 保持 ref-only 与 Codex permission/approval/sandbox authority；current renderer pixels另行。 |
| P1 | Cross-thread user coordination | Rail 有可见键盘入口；App Server list/read/resume/fork/archive/unarchive/start/steer；同 key 返回首个 receipt/result、`ok=true`；cross-host 当前明确 unavailable。 | `dd67a50c1...` candidate已闭合 visible entry、canonical directory、App Server lifecycle、flexible advisory、running steer、首结果 replay、typed pending requests 与无额外 OPL confirmation；cross-host 返回 unsupported。 | `source_implemented`, `pixel_unverified` | 补 current pixels/package；独立非紧急 queue 未实现，不得把 unsupported 包装成 handoff success。 |
| P1 | Model-triggered coordination | 模型通过同一 host adapter调用 list/read/resume/fork/archive/unarchive/start/steer。 | 当前ordinary AionUI conversation未在 `thread/start` 注册 experimental dynamic tools；rail/user dispatch不构成实现。 | `source_missing`, `not_applicable` | 保持required target；统一ordinary thread-start owner后闭合registration、`item/tool/call`和turn result，不新造runtime/store。 |
| P1 | Local / Worktree lifecycle | Home 新任务选择 Local/Worktree 与 starting branch；既有同主机 `not_loaded`/`idle` task 在 Environment 双向 handoff，真实 cwd 先于 projection 更新。 | `dd67a50c1...` candidate已实现 same-host 子集：复用 `gitWorkspace` 与 `thread/settings/update`，使用 `opl_workspace_handoff.v1`，projection failure best-effort 回滚 cwd，worktree默认保留复用。 | `source_partial`, `pixel_unverified` | Snapshot/restore、cleanup UI deferred，cross-host unsupported，完整 lifecycle不得宣称完成。 |
| P1 | Review pane | 复用 Files/Changes；四类 target、inline/detached、PR context、stage/commit/push；缺失能力显示 unavailable。 | `dd67a50c1...` candidate已实现四类 `review/start` target、inline/detached delivery、PR context、stage/commit/push 和 `gh` unavailable；尚无 `last_turn` section 与 line-level inline comment UI/投递。 | `source_partial`, `pixel_unverified` | 保持thin adapter；补`last_turn`和line-level inline comments，不恢复legacy equal-weight Review tab。 |
...
| Project task 与 projectless conversation | `aligned_contract` | `source_implemented` | `pixel_unverified` | `current_contract_deviation` | `source_partial` | `pixel_unverified` | `dd67a50c1...` 保持无 workspace text/attachment、任意 file/directory picker、paste/drop 与 `/open` 可用，访问只服从 Codex permission/approval/sandbox。 |
...
| 一个项目对应 N 个 App Server threads | `aligned_contract` | `source_implemented` | `pixel_unverified` | `candidate_target` | `source_partial` | `pixel_verified` | `dd67a50c1...` 以 `thread/list/read/resume` 为 canonical directory；本地 history只作可重建cache。 |
| 对话 search/pin/rename/archive/restore/delete/reset 与独立 Archived | `aligned_contract` | `source_implemented` | `pixel_unverified` | `current_contract_deviation` | `source_missing` | `pixel_unverified` | `dd67a50c1...` 将rename/archive/restore/delete映射App Server；pin仅UI metadata，local reset不冒充history reset。 |
...
| Environment popover 与 workspace surfaces 分离 | `aligned_contract` | `source_implemented` | `pixel_unverified` | `current_contract_deviation` | `source_partial` | `pixel_unverified` | `dd67a50c1...` 的Environment使用live Git inspection并承载同主机空闲task locality handoff；Files/Preview只在用户或任务需要时出现。 |
...
| Artifact Markdown/PDF/Mermaid/Code/KaTeX preview | `aligned_contract` | `source_implemented` | `pixel_unverified` | `candidate_target` | `source_partial` | `pixel_unverified` | `dd67a50c1...` 已覆盖用户显式绝对本地路径、workspace project ref与拒绝traversal/非法scheme/静默读取；renderer pixels仍独立。 |
| Local/Worktree new task 与同主机 idle handoff | `aligned_contract` | `source_partial` | `pixel_unverified` | `candidate_target` | `source_not_assessed` | `pixel_unverified` | `dd67a50c1...` 覆盖same-host managed create/reuse、`thread/settings/update`、metadata与rollback；完整lifecycle仍缺snapshot/cleanup，cross-host unsupported。 |
| Review 复用 Files/Changes diff surface | `aligned_contract` | `source_partial` | `pixel_unverified` | `candidate_target` | `source_not_assessed` | `pixel_unverified` | `dd67a50c1...` 覆盖四targets、inline/detached、PR context、stage/commit/push与`gh` unavailable；缺`last_turn`和line-level inline comments。 |
...
- **Core pixels：** 8 场景只覆盖历史 `0ebc1fdd...` 的 Home、conversation、composer/model、
  Environment、Files、mobile action sheet 与 mobile Preview；candidate `dd67a50c1...` 尚未重拍，
...
- **Cross-thread user coordination：** Shell `dd67a50c1...` 已实现 visible entry、canonical directory、
  unarchive、首结果 idempotency replay、same-host routing 和 interactive server-request pending flow；
  cross-host当前返回 unsupported，独立 queue未实现，只有 future remote evidence 完成后才能提升。
- **Model host tool：** required target；当前 AionUI source missing。User rail/detail、focused DOM或
  delivery audit都不能替代dynamic-tool registration与`item/tool/call` round-trip evidence。
- **Artifact ref adapter：** `dd67a50c1...` 已覆盖用户显式任意绝对本地路径、workspace ref、双 scope
  policy和非法输入拒绝；current renderer pixels/package仍需独立证据。
- **Codex locality / Review：** `dd67a50c1...` 已实现same-host Home worktree与idle task handoff，但
  snapshot/cleanup deferred、cross-host unsupported。Review已实现四 target、inline/detached、PR
  context、stage/commit/push与`gh` unavailable，仍缺`last_turn`和line-level inline comments。
