...
本轮 fresh B0 slice 为 `B0-08/B0-11/B0-12/B0-13/B0-14 source_implemented`。
...
| P0 | Project hierarchy | Project affinity 为零或一；Project/directory 提供 thread 的初始 cwd、projectless 一次性 adoption、可见 metadata 与分组，不拥有 session、context 或 artifact；rail canonical history/actions 来自 App Server。 | 公共 App Server/IPC 通道执行 `thread/settings/update.cwd -> thread/read` exact readback，成功后才写本地 projection；canonical cwd 覆盖 stale cache，失败保持 projectless，已有 cwd 禁止改绑。 | `aligned_contract` | `source_implemented` | `pixel_unverified` | `install_unverified` | `release_unverified` | 保持无 Git-origin collapse、通用重绑、writable-root mutation、pending/receipt/Handoff 层；后续只补 Pixel/Install/Release。 |
...
| P1 | Session Project-affinity boundary | 新任务选择初始 cwd；仅 `custom_workspace=false` 或无 canonical recorded cwd 的 session 可一次归口；已有 cwd 不任意换组，turn cwd、shell `pwd`、显式输入与 writable roots 不反写 rail metadata。 | Drag/menu 通过公共 App Server adapter 先 update、再 exact read，最后提交本地 projection；失败保持 projectless，canonical cwd 修复 stale cache，已有 recorded cwd 阻止 reassignment。 | `aligned_contract` | `source_implemented` | `pixel_unverified` | `install_unverified` | `release_unverified` | 后续只做 Pixel/Install/Release；保持无私有 adoption RPC、`workspace_handoff`、managed Worktree/Handoff、第二 client 或任意 bound reassignment。 |
...
|   Projectless session 一次性归入一个目录组 | `aligned_contract` | `source_implemented` | `pixel_unverified` | `candidate_target` | `source_missing` | `pixel_unverified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | AionUI 通过公共 App Server/IPC 执行 `thread/settings/update.cwd -> thread/read` exact readback，成功后才提交 projection；失败保持 projectless，已有 cwd 不改绑。Native 仍缺；turn `pwd`、writable roots、pending/receipt/Handoff 均保持独立。   |
...
|   独立 context bar 设置新 session 初始 cwd；`+` 打开可搜索能力 palette | `aligned_contract` | `source_implemented` | `pixel_unverified` | `candidate_target` | `source_missing` | `pixel_unverified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | AionUI 已把 cwd 放到 composer 上方独立 context bar；`+` 始终展开 composer-width、可搜索/分组/滚动的 files、folders 与真实 allowlisted capabilities palette。Native 当前仍没有真实目录切换闭环。   |
