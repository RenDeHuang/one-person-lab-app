...
Snapshot basis: `2026-07-17` fresh B0/R1/U1 source audit; earlier exact-cohort pixels retain their own dates and hashes.
...
| `B0-12` Scheduled Tasks | `aligned_contract` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | Machine refs 已扩展为 `app-gui-product-contract#scheduled_tasks_policy`、page state、`app-product-profile#companion_payloads.native_automation` 与 `app-shell-adapter#upstream_intake.managed_agent_api_contract.write_contracts.cron`。合同锁定单一 carrier scheduler/store、ordinary Sider、fixed Codex exact `Assistant.id`、局部 fail-open 与 legacy executor preservation；AionUI 仍需挂载 Sider 并收敛 create/edit selector。 |
...
| `R1-06` | OPL bundle、update、deep link、feedback/support。安装、唤起、更新和求助必须是同一产品。 | `aligned_contract` | `L1 profile + L2 bridge/adapter` | `source_implemented` | `pixel_unverified` | `install_unverified` | `release_unverified` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | AionUI candidate 已迁移为单一 `opl://navigate?route=...`，只允许 App-owned exact route；cold argv、second-instance、macOS `open-url` 共用校验路径，renderer ready 后 pull pending payload。credential action、额外/重复参数、encoded payload 与 secret-like value 均局部拒绝并只记录 reason code；process/renderer focused、full Shell、type/format/i18n/lint 已通过。尚未吸收 canonical main，Pixel/Install/Release 不回填；Native 仍需正式 updater/deep-link/support。 |
...
| `U1-05` | Docker/WebUI 同产品语义。WebUI 是 OPL 的部署入口，不能成为另一个产品。 | `aligned_contract` | `upstream reuse + L2 transport adapter` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | 补 Web Storage/core route parity 与当前 Docker cohort；Desktop-only 安全差异必须显式。 |
| `U1-06` | OPL 数据、缓存、包体空间与安全清理。长期使用必须可预览、可确认、可恢复。 | `current_contract_deviation` | `L2 bridge/adapter + L3 composition` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | `source_missing` | `pixel_unverified` | `install_unverified` | `release_unverified` | App/Framework/host 先补 Agent Package store 与 WebUI volume 的 owner inventory/action contract/projection，再由 Shell/Web消费；禁止 Shell 直接改 package 或泛化调用 `docker system prune`。 |

Source 状态说明：AionUI `R1-01..05`、`U1-01..04` 为 `source_implemented`；
`R1-06`、`U1-05..06` 为 `source_partial`，没有完全 missing。Native 的 `R1-03`、
...
U1-06 仍有 contract deviation；任何实现顺序都不能把不同状态压成一个 aggregate completion 值。
...
- Current Shell source cohort：symbolic `session_workspace_minimal_current_source_cohort`，由 active checkout 的
  ...
  feedback/avatar/help 与窄窗 Access。Workspace selector 只设置新 session 初始 cwd；已绑定 session 不提供
  cwd 重绑或 managed Worktree/Handoff。Projectless 一次性 adoption 已是 contract target，但当前 source cohort
  尚未实现。Review 保留四类 target、`Last turn` 与 custom
...
| P0 | Project hierarchy | Project affinity 为零或一；Project/directory 提供 thread 的初始 cwd、projectless 一次性 adoption、可见 metadata 与分组，不拥有 session、context 或 artifact；rail canonical history/actions 来自 App Server。 | App Server canonical thread directory/actions 与单一 directory identity 已有；但一次性 adoption 仍只写本地 conversation projection，尚未执行合同要求的 `thread/settings/update.cwd -> thread/read`。 | `aligned_contract` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | 用既有 App Server adapter 补 canonical cwd update/readback；不得恢复 Git-origin collapse、通用重绑、writable-root mutation、pending/receipt/Handoff 层。 |
| P0 | Home / New task | 与 conversation 共用 chat canvas/composer，不是 dashboard；统一 `+` 菜单只设置新任务初始 cwd，未选时无占位，选中后为可移除 chip。 | Composer-first Home、全部用户可见 starters、stable order、responsive wrap 与 package readiness 已实现；Local/Worktree 与 starting branch 控件已删除。 | `aligned_contract` | `source_implemented` | `pixel_verified` | `install_unverified` | `release_unverified` | Current desktop/mobile pixels覆盖Home、rail与mobile action sheet。 |
...
| P0 | Composer | 统一 `+`、access、model/reasoning 与 send-stop 位于发送决策点；文件、目录、新 session cwd、allowlisted Skill 与真实连接共用一个菜单。 | Desktop/mobile 共用 App resolver/profile 和菜单内容；Home 创建、ACP/AionRS initial send 与会话内 send 失败均复用现有 draft store 恢复 prompt/附件，且不覆盖等待期间新增输入；legacy intelligence proxy controls 已移除。 | `aligned_contract` | `source_implemented` | `pixel_verified` | `install_unverified` | `release_unverified` | Current pixels覆盖desktop controls/model menu与mobile action sheet，但不单独证明失败恢复；不恢复第二 provider/service truth。 |
...
| P1 | Session Project-affinity boundary | 新任务选择初始 cwd；仅 `custom_workspace=false` 或无 canonical recorded cwd 的 session 可一次归口；已有 cwd 不任意换组，turn cwd、shell `pwd`、显式输入与 writable roots 不反写 rail metadata。 | Drag/menu/local fail-open 骨架已有，但 canonical `thread/settings/update.cwd`、exact `thread/read`、成功后才提交本地 projection 的顺序尚未实现。 | `aligned_contract` | `source_missing` | `pixel_unverified` | `install_unverified` | `release_unverified` | 先闭合 App Server update/readback 与失败/已有 cwd 负向测试，再做 Pixel、Install、Release；保持无私有 adoption RPC、`workspace_handoff`、managed Worktree/Handoff、第二 client 或任意 bound reassignment。 |
...
| X0-03/04 | Optional external resource owner routes | Hosted Workspace、Fabric、HPC、Console 只在 canonical owner/backend projection 存在时显示 refs/route；空 projection 无 group、anchor 或占位。 | App contract/fixture/validator 已对齐；AionUI `ResourcesSettings` 仍以聚合开关同时挂载 workspace/external 两组，并保留 hosted workspace 默认宣传文案。 | `aligned_contract` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | Shell owner 分组独立条件渲染并删除 hosted promise copy；不触碰本机浏览器、Docker/WebUI 基线。 |
...
|   Rail 顶部 New task/Archived；Runtime 仅为条件保留的 X0-01 入口，底部 account/help/Settings | `aligned_contract` | `source_implemented` | `pixel_unverified` | `not_claimed` | `source_not_assessed` | `pixel_unverified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | AionUI 当前保留 Runtime，产品分类为 `retained_optional_x0_owner_route` 且默认门不要求它；Native phase-1 不声明或承担 Runtime parity。   |
|   Workspace-initialized 与 projectless session | `aligned_contract` | `source_implemented` | `pixel_unverified` | `current_contract_deviation` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | `b2c05a1c...` 保持无 workspace text/attachment、任意 file/directory picker、paste/drop 与 `/open` 可用；current pixels使用workspace fixture，不证明projectless path。   |
|   Projectless session 一次性归入一个目录组 | `aligned_contract` | `source_missing` | `pixel_unverified` | `candidate_target` | `source_missing` | `pixel_unverified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | AionUI 有拖动、菜单、local projection 与 fail-open 骨架，但缺 canonical `thread/settings/update.cwd -> thread/read`；Native 也缺。两者都必须只允许 projectless→bound once，已有 cwd 不改绑，并保持 turn `pwd`、writable roots、pending/receipt/Handoff 独立。   |
...
|   统一 `+` 菜单设置新 session 初始 cwd | `aligned_contract` | `source_implemented` | `pixel_unverified` | `candidate_target` | `source_missing` | `pixel_unverified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | AionUI 在同一菜单提供 file/folder/cwd/Skill/可用连接；cwd 选择只设置新 session 初始值，不创建 session owner；Native 当前没有真实目录切换闭环。   |
...
|   一个目录组可展示 N 个独立 App Server sessions | `aligned_contract` | `source_implemented` | `pixel_unverified` | `candidate_target` | `source_partial` | `pixel_verified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | AionUI 以 `thread/list/read/resume` 为 canonical directory；分组只来自各 session recorded cwd，不建立目录所有权，完整directory/actions仍由source/protocol gates证明。   |
|   对话 search/pin/rename/archive/restore/delete/reset 与独立 Archived | `aligned_contract` | `source_implemented` | `pixel_unverified` | `current_contract_deviation` | `source_missing` | `pixel_unverified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | `b2c05a1c...` 将rename/archive/restore/delete映射App Server；pin仅UI metadata，current matrix未打开这些actions。   |
...
|   Terminal/Browser 从 Environment 或任务需要按需打开，无 Runtime duplicate | `aligned_contract` | `source_implemented` | `pixel_unverified` | `current_contract_deviation` | `source_partial` | `pixel_verified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | Source/DOM 已证明入口与默认关闭；本轮 core manifest 未单独打开 Terminal/Browser。   |
...
|   Artifact Markdown/PDF/Mermaid/Code/KaTeX preview | `aligned_contract` | `source_implemented` | `pixel_unverified` | `candidate_target` | `source_partial` | `pixel_unverified` | `install_unverified` | `release_unverified` | `install_unverified` | `release_unverified` | `b2c05a1c...` 覆盖当前 session ref、绝对本地路径与非法输入拒绝；current Preview pixel不证明各renderer内容。   |
...
- **Session locality / Review：** Current source cohort 已有 Home 新任务初始 cwd 和 projectless conversation，尚缺
  projectless 一次性 adoption；已绑定 session 不提供任意 cwd 重绑、managed Worktree create/reuse 或 handoff。Review owner topic
