...
- Current Shell source cohort：
  `opl-aion-shell@285deb60b0cd0b5ffd1bae77bde526236a12aa17`（基于main `6639744c...`）。它合并当前 Runtime V2 main、Gateway
  ...
  avatar/help 与窄窗Access布局。Main/remote最终currentness仍从
...
- Current source gates：Shell `285deb60b...` full active-shell `302 files pass / 1 skip`、
  `2415 pass / 9 skip`；TypeScript、1548-file format、i18n与lint `0 errors / 888 warnings`通过；
  avatar/help owner focused logic `28 pass`、DOM `5 pass`。App release-boundary
  `351 pass / 2 platform skip`。这些计数均绑定final exact source或该exact包含的focused owner
...
- Latest package-bound visual evidence（相对current source为historical）：
  ...
  Manifest明确保持 `parity_1_to_1=false` 与 `release_ready=false`，不得改绑到`285deb60b...`。
...
| P1 | Local / Worktree lifecycle | Home 新任务选择 Local/Worktree 与 starting branch；既有同主机 `not_loaded`/`idle` task 在 Environment 双向 handoff，真实 cwd 先于 projection 更新；remove前必须可恢复。 | `285deb60b...` 复用 `gitWorkspace` 与 `thread/settings/update`，并以 `opl_worktree_snapshot_receipt.v1` Git ref持久化HEAD/branch或detached/index/tracked/untracked/ignored user files，remove失败恢复，restore冲突typed fail。 | `source_partial`, `pixel_unverified` | Same-host lifecycle已闭合；cross-host required target因Codex Remote Connections/host-handoff owner协议面缺失而blocked。 |
| P1 | Review pane | 复用 Files/Changes；四类 target、inline/detached、PR context、stage/commit/push；缺失能力显示 unavailable。 | `285deb60b...` 已实现四类 `review/start` target、inline/detached、PR context、stage/commit/push、`gh` unavailable、Last turn，并把非custom focus经同client `turn/steer(expectedTurnId)`送回原Review turn。 | `source_partial`, `pixel_unverified` | Line-level comments等待typed Codex file/line request；禁止local annotation store或假成功，不恢复legacy equal-weight Review tab。 |
...
| Local/Worktree new task 与同主机 idle handoff | `aligned_contract` | `source_partial` | `pixel_unverified` | `candidate_target` | `source_not_assessed` | `pixel_unverified` | `285deb60b...` 覆盖same-host managed create/reuse、handoff、durable snapshot-before-remove（含ignored user files）、cleanup rollback与receipt restore；cross-host为owner blocker。 |
| Review 复用 Files/Changes diff surface | `aligned_contract` | `source_partial` | `pixel_unverified` | `candidate_target` | `source_not_assessed` | `pixel_unverified` | `285deb60b...` 覆盖四targets、inline/detached、PR context、stage/commit/push、`gh` unavailable、Last turn与same-turn focus steer；line-level comments为protocol blocker。 |
...
- **Codex locality / Review：** `285deb60b...` 已实现same-host Home worktree、idle task handoff、durable
...
- **Narrow Settings：** `285deb60b...` ancestry 将Access摘要卡的双列断点从`md`提高到`xl`，960px窗口保持
