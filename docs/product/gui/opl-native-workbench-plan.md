...
| 5 | Codex app-server conversation bridge | Candidate uses Codex app-server JSON-RPC (`initialize`, `model/list`, `thread/start`, `turn/start`, model/effort overrides, streaming delta notifications, `turn/completed`, and resume-capable thread ids) instead of a shell-owned one-shot `codex exec` wrapper. The App-owned allowlist remains the visible model truth. With a catalog present, unadvertised fixed alternatives are disabled while the App default route remains available. Auto is the complete latest-and-strongest policy, currently `gpt-5.6-sol` with `xhigh`; choosing another reasoning level pins the current model and exits Auto. | Native bridge compile, app-server protocol smoke, candidate package manifest, model-selection behavior validation, and a live `gpt-5.6-sol` / `xhigh` turn probe. |
...
| --- | --- | --- |
| Implemented | Codex chat runtime path | Source and behavior validators require `codex app-server --stdio`, `initialize`, `model/list` filtering for fixed alternatives with an App-default route exception, complete Auto `gpt-5.6-sol` / `xhigh` semantics, manual-reasoning pinning, stale fixed-selection normalization, `thread/start`, `turn/start`, `model` / `effort` overrides, `item/agentMessage/delta`, `turn/completed`, and `thread/resume` markers across the bridge and native host. |
