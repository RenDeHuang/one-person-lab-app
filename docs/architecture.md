...
Settings boundary 也遵循同样拆分。普通 Settings navigation 是 General、Access、Agents & Capabilities、Local Environment、Storage、Appearance、Advanced、About & Updates。Overview、runtime、system、model、agent、assistants、skills-hub、tools、display、webui、pet 等 legacy upstream routes redirect 到 App-owned pages。Storage owns local data lifecycle inventory and cleanup controls; Shell 仍可保留 diagnostic 或 redirected sub-content 的实现组件，但这些组件不定义普通 App navigation 或 product authority。
