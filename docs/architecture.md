...
| OPL Packages | MAS/MAG/RCA/OMA/BookForge/ScholarSkills/OPL Flow packages. Domain agents use `domain_agent_package`; ScholarSkills uses `framework_capability_package`; OPL Flow uses `workflow_plugin_package`. Ordinary users update through GHCR OCI artifacts with a single rolling `latest` pointer after daily gated promotion where release-published; immutable version tags plus resolved OCI digests are the installed truth. Clean managed roots may be silently updated by OPL maintenance; dirty checkouts, developer checkouts, idempotency locks, verification failures, permission changes, and manual-required conditions fail closed instead of being overwritten. |
