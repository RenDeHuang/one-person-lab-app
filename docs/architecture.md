...
| OPL Runtime Payload / Fabric | Runtime foundation needed to launch or recover OPL. User-facing grouping is Agent Execution Core (Codex executor, Temporal task runner, OPL Framework runtime), Environment Materializer (managed language runtimes, package/env resolvers, env cache, isolated prefixes, and receipts), and OPL System Bridge (native helper only where platform boundaries require it). `runtime_substrate` remains the machine id. Runtime Payload follows stable/nightly/release-host routing, not the Agent Package rolling `latest` pointer. The OPL Framework core has one active carrier: explicit Developer Mode uses the selected checkout; Homebrew-origin installs use a compatible system `opl` Formula once published; DMG/direct installs use the App-managed private install under `~/.opl/one-person-lab`. During Formula pre-publication only, a Caskroom-detected App may use that private install with `pre_formula_transition`; once Formula exists, incompatibility fails closed and cannot fall back. Other system PATH and global tools remain diagnostic-only. |
