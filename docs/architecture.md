...
| Capability Packages | MAS/MAG/RCA/OMA/BookForge/ScholarSkills OPL Packages. Ordinary users update through GHCR `latest` after daily gated promotion; immutable version tags plus resolved digests are the installed truth. Clean managed roots may be silently updated by OPL maintenance; dirty checkouts, developer checkouts, idempotency locks, verification failures, permission changes, and manual-required conditions fail closed instead of being overwritten. |
