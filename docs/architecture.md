...
The runtime page contract is display and routing only. It consumes `opl app state --profile fast --json` as the summary and refresh source, keeps `opl app state --profile full --json` for explicit full-state diagnostic or release evidence, and uses whitelisted `opl app action execute` routes for operator-selected actions. Full Framework drilldown remains an on-demand exception. Runtime truth, action execution authority, domain verdicts, memory bodies, and artifact bodies remain outside the App. `contracts/app-runtime-bridge.json#progress_delta_projection` binds progress display to the OPL shared progress projection classification fields: `deliverable_progress_delta`, `platform_repair_delta`, and `progress_delta_classification`. The page-state matrix mirrors that bridge-level projection. Platform repair is displayed as infrastructure repair, never as deliverable, paper, manuscript, or submission progress.

The default Runtime page attention model is owner-action and project-progress first. The ordinary view answers which project is running or waiting, what owner action or blocker comes next, and whether the next delta is deliverable progress, platform repair, human gate, typed blocker, or stop-loss. Operator summary, safe actions, refs-only evidence, and full ledger detail are secondary diagnostic disclosures. A release/user-path evidence bundle can support the same App release cohort and release-owner review, but it cannot by itself promote stable/latest, prove domain readiness, or prove OPL family production readiness.
