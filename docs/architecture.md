...
| OPL Runtime Payload / Fabric | App-owned runtime foundation needed to launch or recover OPL. User-facing grouping is Agent Execution Core (Codex executor, Temporal task runner, OPL Framework runtime), Environment Materializer (managed language runtimes, package/env resolvers, env cache, isolated prefixes, and receipts), and OPL System Bridge (native helper only where platform boundaries require it). `runtime_substrate` remains the machine id. Runtime Payload follows stable/nightly/release-host routing, not the Agent Package rolling `latest` pointer. Ordinary defaults use the App-owned runtime; system PATH, Homebrew, global tools, and developer checkouts are diagnostic or explicit expert opt-in unmanaged sources. |
| Agent Packages | MAS/MAG/RCA/OMA/BookForge/ScholarSkills OPL Packages. Ordinary users update through GHCR OCI artifacts with a single rolling `latest` pointer after daily gated promotion; immutable version tags plus resolved OCI digests are the installed truth. Clean managed roots may be silently updated by OPL maintenance; dirty checkouts, developer checkouts, idempotency locks, verification failures, permission changes, and manual-required conditions fail closed instead of being overwritten. |
