...
- Release 技术双轨只改变执行位置：本地轨用于开发、调试、快速构建、同件验收及明确授权的发布 fallback，远端轨在流程稳定后作为常规发布与持续证明路线；两轨必须使用同一 frozen cohort，并保持公共资产、AI notes、安装与自动更新行为完全等价。Standard 六资产和 notes 齐全即可成为 Latest，Full 可随后只追加且不得进入 updater metadata；所有构建与验收使用 release 自有只读 worktree，不得锁住 canonical `main` 或无关开发 worktree。
- Release 低层 `desktop-release*` 与 `opl-first-run-vm` workflow 禁止手工 mutation 和 rerun。当前 Stable critical path 仅允许唯一 executor 通过 canonical `release:stable` controller 对 `desktop-release.yml` 与 `desktop-release-promote.yml` 各提交至多一次 scoped administrator one-shot dispatch；每次提交前必须在唯一 session 中 durable记录 planned 与 dispatching，绑定 frozen cohort、canonical `main` controller SHA、exact payload digest 与 attempt id。未知提交结果或 run 暂不可见时只能 read-only reconcile，禁止 redispatch；admin one-shot attempt 禁止 cancel。Full/add-on 的构建、同件验收与明确授权的 exact-artifact 本地发布 fallback 遵循 `release_execution_tracks`，不得因为从远端切到本地而重建；Full/add-on 远端 mutation、cleanup 及其他 mutation 仍只允许隔离 broker。未部署 broker是发布后 hardening debt，不是当前 Standard admission硬门。Monitor 只允许 GET/readback。失败必须先 reconcile 并记录 failure taxonomy；没有 durable receipt 不得手工续跑。Compiled expectation 或 verification harness SHA 变化必须冻结新 cohort，不允许仅凭 changed-path allowlist 复用 artifact。发布对话或代理树不得作为 scheduler、watcher 或 state store：禁止递归监控/审计子树和重复 polling，所有接管只读 canonical session 并执行一次 typed reconcile；已有 owner 时其他 task 必须停止路由和等待。除上述 scoped one-shot外，日常 Codex credential保持 Actions read-only，不能绕过受保护的 canonical `main` / release-control path直接改写 verifier。
