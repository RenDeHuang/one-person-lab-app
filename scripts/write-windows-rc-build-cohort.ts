...
export type WindowsRcBuildCohortV1 = {
  ...
  runtime: {
    execution_substrate: 'bundled_native_windows_aioncore_and_codex_acp';
    wsl2_only_terminal_claim: false;
...
function collectTreeFiles(root: string, relative = ''): FileIdentity[] {
  ...
  return fs.readdirSync(directory, { withFileTypes: true })
    ...
    .flatMap((entry) => {
      ...
      if (entry.isSymbolicLink()) throw new Error(`Packaged Windows tree must not contain symlinks: ${childPath}`);
...
function packagedTreeIdentity(root: string) {
  ...
  if (!stat.isDirectory()) throw new Error(`Packaged Windows tree is not a directory: ${root}`);
...
function exactSha(value: string, label: string): string {
  ...
  if (!gitShaPattern.test(normalized)) throw new Error(`${label} must be an exact 40-character Git SHA.`);
  return normalized;
}

function optionalExactSha(value: string | undefined, label: string): string | null {
...
function findFiles(root: string, predicate: (relativePath: string) => boolean): string[] {
...
export function buildWindowsRcBuildCohort(input: {
  ...
  if (input.platform !== 'win32') throw new Error('Windows RC cohort platform must be win32.');
  if (input.arch !== 'x64' && input.arch !== 'arm64') throw new Error('Windows RC cohort arch must be x64 or arm64.');
  if (!versionPattern.test(input.version)) throw new Error('Windows RC display version must match <semver>-rc.<positive integer>.');
  if (!/^[1-9]\d*$/.test(input.actionsRunId)) throw new Error('GitHub Actions run id must be a positive integer.');
  if (!/^[1-9]\d*$/.test(input.actionsRunAttempt)) throw new Error('GitHub Actions run attempt must be a positive integer.');
  if (!input.actionsArtifactName.trim()) throw new Error('GitHub Actions artifact name is required.');

  const packagedTree = path.resolve(input.packagedTreePath);
  const runtimeKey = `${input.platform}-${input.arch}`;
  const runtimeRoot = path.join(packagedTree, 'resources', 'bundled-aioncore', runtimeKey);
  const aioncorePath = path.join(runtimeRoot, 'aioncore.exe');
  ...
  const codexPath = exactFile(
    findFiles(runtimeRoot, (relativePath) =>
      relativePath.includes(`/node_modules/${codexPackage}/vendor/`) && relativePath.endsWith('/bin/codex.exe')),
    `${codexPackage} binary`,
  );

  for (const required of [input.installerPath, aioncorePath, runtimeManifestPath, managedManifestPath, codexPath]) {
    if (!fs.existsSync(required)) throw new Error(`Windows RC cohort input is missing: ${required}`);
  }
  const runtimeManifest = JSON.parse(fs.readFileSync(runtimeManifestPath, 'utf8'));
  if (runtimeManifest.platform !== input.platform || runtimeManifest.arch !== input.arch) {
    throw new Error('Bundled AionCore manifest platform/arch does not match the Windows RC target.');
  }
  ...
  const codexTools = Array.isArray(managedManifest.acpTools)
    ? managedManifest.acpTools.filter((tool: { slug?: string }) => tool?.slug === 'codex-acp')
    : [];
  if (codexTools.length !== 1) throw new Error('Managed resources must contain exactly one codex-acp tool.');
  ...
  return {
    ...
    source: {
      app: { sha: exactSha(input.appSha, 'App SHA'), tree: exactSha(input.appTree, 'App tree') },
      shell: { sha: exactSha(input.shellSha, 'Shell SHA'), tree: exactSha(input.shellTree, 'Shell tree') },
      framework_sha: optionalExactSha(input.frameworkSha, 'Framework SHA'),
    },
    target: {
      platform: 'win32',
      arch: input.arch,
    ...
    artifact: relativeFileIdentity(path.dirname(input.installerPath), input.installerPath),
    ...
    runtime: {
      ...
      managed_resources_manifest: relativeFileIdentity(packagedTree, managedManifestPath),
...
function main() {
  ...
  for (const key of [
    'output', 'installer', 'packaged-tree', 'app-sha', 'app-tree', 'shell-sha', 'shell-tree',
    'version', 'platform', 'arch', 'actions-run-id', 'actions-artifact-name',
  ] as const) {
  ...
  fs.writeFileSync(values.output!, `${JSON.stringify(cohort, null, 2)}\n`, 'utf8');
  process.stdout.write(`${JSON.stringify({ status: 'written', output: values.output, cohort })}\n`);
}
