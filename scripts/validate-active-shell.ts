...
import { assertDeepEqualJson, assertIncludesAll, readJson } from './validate-active-shell/assertions.ts';
import {
...
} from './validate-active-shell/shared-contract-validators.ts';
...
function validateAppGuiProductContract(guiContract, releaseChannel, installExposurePolicy) {
  ...
  for (const explanation of [
    ...
    'whether a module comes from the App/CLI-managed stable package channel',
    'whether a module comes from the App/CLI-managed nightly package channel',
  ...
  if (
    modulePathPolicy.ordinary_user_source !== 'app_cli_managed_stable_package_channel' ||
