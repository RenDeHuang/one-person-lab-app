...
function assertManagedOutputPath(input: {
  ...
  const isDefault =
    input.outDir === (input.mode === "full-dmg" ? defaultFull : defaultLocal);
...
function managedOutputStage(outDir: string) {
  ...
  return fs.mkdtempSync(
    path.join(parent, `.${path.basename(outDir)}.staging-`),
  );
}
...
function promoteManagedOutput(stagingDir: string, outDir: string) {
  ...
  const backupRoot = fs.mkdtempSync(
    path.join(parent, `.${path.basename(outDir)}.backup-`),
  );
...
function parseOptions(argv: string[]) {
  ...
  if (
    ...
    !["local-app", "full-dmg"].includes(String(mode))
  ) {
    throw new Error(
      "Usage: manual-latest-build.ts <local-app|full-dmg> [options]",
    );
  }
  ...
  const defaultOutDir = values["print-plan"]
    ...
    : mode === "full-dmg"
      ? path.join(
          os.homedir(),
          "Downloads",
          `One-Person-Lab-Manual-Full-${version}`,
        )
  ...
  return {
    ...
    installPath: path.resolve(
      values["install-path"] || "/Applications/One Person Lab.app",
    ),
...
function resolveUiUxRoot(workspaceRoot: string, explicit: string | null) {
  ...
  if (!found)
    throw new Error(
      `UI UX Pro Max source is missing; checked: ${candidates.join(", ")}`,
    );
  return found;
}
...
function repoSnapshots(
  ...
  const owners = Object.fromEntries(
    Object.entries(OWNER_REPOS).map(([packageId, repoName]) => [
      packageId,
      snapshotDevelopmentRepo(
        packageId,
        path.join(options.workspaceRoot, repoName),
      ),
    ]),
  ) as Record<string, RepoSnapshot>;
  const uiUxRoot = resolveUiUxRoot(
    options.workspaceRoot,
    options.uiUxProMaxRoot,
  );
  const uiUxRepoRoot = commandResult(
    "git",
    ["-C", uiUxRoot, "rev-parse", "--show-toplevel"],
    {
      capture: true,
      timeoutMs: 30_000,
    },
  ...
  if (!uiUxRepoRoot)
    throw new Error(`Cannot resolve UI UX Pro Max repository: ${uiUxRoot}`);
  return {
...
function runBuild(
  ...
  const args = [
    ...
    options.outDir,
    "--framework-root",
    snapshots.framework.root,
    "--gui-root",
    snapshots.shellRoot,
    "--mas-root",
    snapshots.owners.mas.root,
    "--mas-scholar-skills-root",
    snapshots.owners["mas-scholar-skills"].root,
    "--mas-scholar-skills-ref",
    snapshots.owners["mas-scholar-skills"].head,
    "--mag-root",
    snapshots.owners.mag.root,
    "--rca-root",
    snapshots.owners.rca.root,
    "--meta-agent-root",
    snapshots.owners.oma.root,
    "--bookforge-root",
    snapshots.owners.obf.root,
    "--opl-flow-root",
    snapshots.owners["opl-flow"].root,
    "--officecli-root",
    upstreams.officecli.source_root,
    "--officecli-bin",
    upstreams.officecli.binary,
    "--mineru-open-api-bin",
    upstreams.mineru_open_api.binary,
    "--ui-ux-pro-max-root",
    snapshots.uiUxRoot,
    "--temporal-cli-bin",
    upstreams.temporal.binary,
    "--temporal-cli-archive",
    upstreams.temporal.archive,
  ...
  commandResult(process.execPath, args, {
    ...
    env: {
      ...
      OPL_MANUAL_LOCAL_BUILD_ID:
        options.mode === "local-app" ? buildIdentity.local_build_id : "",
      OPL_MANUAL_LOCAL_SOURCE_PROVENANCE_SHA256:
        options.mode === "local-app"
          ? buildIdentity.source_provenance_sha256
          : "",
      OPL_MANUAL_LOCAL_SOURCE_LOCK_SHA256:
        options.mode === "local-app" ? buildIdentity.source_lock_sha256 : "",
    },
...
function fullDmgEvidence(
  ...
  const manifestPath = requireFile(
    path.join(buildOutDir, names.manifest),
...
function main() {
  ...
  const workRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "opl-manual-latest-build-"),
  );
  const buildOutDir = options.printPlan
    ? options.outDir
    : managedOutputStage(options.outDir);
  let completed = false;
  ...
  try {
    ...
    const upstreams = prepareLatestUpstreams(
      path.join(options.cacheRoot, "upstreams"),
    );
    const sourceProvenance = {
      ...
      source_policy: {
        ...
        external_companions:
          "latest_official_stable_github_release_digest_verified",
        package_selection:
          "actual_selected_source_commits_recorded_in_full_package_manifest",
      },
      repositories: {
        app: snapshots.app,
    ...
    const stagedSourceLockPath = path.join(
      buildOutDir,
    ...
    if (options.mode === "local-app") {
      assertDevelopmentRepoSnapshotsUnchanged(
        developmentRepoSnapshots(snapshots),
      );
      try {
      ...
      writeJson(
        path.join(buildOutDir, "manual-local-app-installation.json"),
        installation,
      );
    }
    const output =
      options.mode === "full-dmg"
        ? fullDmgEvidence(options, buildOutDir, aioncoreBinding)
        : {
            installed_app: options.installPath,
            installation_receipt: path.join(
              options.outDir,
              "manual-local-app-installation.json",
            ),
          };
    ...
    writeJson(path.join(buildOutDir, "manual-latest-build-receipt.json"), {
      ...
      local_build_id:
        options.mode === "local-app"
          ...
          : null,
      build_identity:
        options.mode === "local-app" ? stampedLocalAppIdentity : null,
      source_lock: sourceLockPath,
      source_lock_sha256: sourceLockSha256,
      output,
      installation,
      ...
      JSON.stringify(
        {
          status:
            options.mode === "local-app"
              ? "manual_latest_local_app_ready"
              : "manual_latest_full_dmg_ready",
          source_lock: sourceLockPath,
    ...
    if (!completed) {
      console.error(
        "Manual latest build did not complete; no success claim was written.",
      );
    }
  }
}
