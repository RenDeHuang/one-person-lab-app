...
import {
  ...
  monotonicSeconds,
  run,
} from "./build-full-first-install-package/process.ts";
...
const MANUAL_LOCAL_SOURCE_PROVENANCE_ENV =
...
function buildFullPublicReleaseManifest(input) {
  return {
    ...
    evidence: {
      ...
      runtime_currentness_probe: readJsonIfExists(
        input.runtimeCurrentnessProbePath,
      ),
      ...
      local_authorization_policy: readJsonIfExists(
        path.join(input.outDir, "full-local-authorization-policy.json"),
      ),
...
function resolveManualLocalAppIdentity(options) {
  ...
  const sourceProvenanceSha256 =
    process.env[MANUAL_LOCAL_SOURCE_PROVENANCE_ENV]?.trim() || "";
  const sourceLockSha256 =
    process.env[MANUAL_LOCAL_SOURCE_LOCK_ENV]?.trim() || "";
  const supplied = [
    localBuildId,
  ...
  if (!options.appOnly) {
    if (supplied) {
      throw new Error(
        "Manual local App identity is allowed only with --app-only",
      );
    }
    return null;
  }
  ...
  if (
    localBuildId !== expected.local_build_id ||
    !/^[0-9a-f]{64}$/.test(sourceLockSha256)
  ) {
    throw new Error(
      "Manual local App build identity does not match its public updater and source lock",
    );
  }
  return {
    ...expected,
...
function main() {
  ...
  options.officeCliRelease = resolveOfficeCliReleaseSource(
    options.officeCliRoot,
    options.officeCliRef,
  );
  ...
  if (options.printRuntimeCacheKeys) {
    console.log(
      JSON.stringify(buildRuntimeCacheKeyReport(options, sources), null, 2),
    );
    return;
  }
  ...
  timings.runtime_materialize = durationSeconds(
    buildStartedAt,
    runtimePreparedAt,
  );
  timings.runtime_cache_materialize = Number(
    prepared.runtime_cache.events
      .reduce((sum, event) => {
        return (
          sum +
  ...
  const runtimeCacheEventsPath = path.join(
    options.outDir,
    artifactNames.runtimeCacheEvents,
  );
  writeJsonFile(runtimeCacheEventsPath, prepared.runtime_cache);
  const runtimeCurrentnessProbePath = path.join(
    options.outDir,
    "full-runtime-currentness-probe.json",
  );
  writeJsonFile(
    runtimeCurrentnessProbePath,
    prepared.runtime_cache.currentness,
  );
  const runtimeNativeTrustPath = path.join(
    options.outDir,
    "full-runtime-native-trust.json",
  );
  ...
  const payloadRoots = syncRuntimePayloadToBuildRoots(
  ...
  if (!options.skipGuiBuild) {
    ...
    timings.shell_build = durationSeconds(
      shellBuildStartedAt,
      monotonicSeconds(),
    );
  } else {
  ...
  const precompressionGatePath = path.join(
    options.outDir,
    "full-precompression-gate.json",
  );
  ...
  timings.precompression_gate = durationSeconds(
    precompressionStartedAt,
    monotonicSeconds(),
  );
  ...
  const runtimeTar = maybeCreateRuntimeTar(
    options,
    prepared.runtimeRoot,
    artifactNames,
  );
  timings.dmg_package_compression = durationSeconds(
    packageCompressionStartedAt,
    monotonicSeconds(),
  );
  ...
  const appBundleTrimPath = path.join(
    options.outDir,
    "full-app-bundle-trim-report.json",
  );
  const packageBoundaryAuditPath = path.join(
    options.outDir,
    "full-package-boundary-audit.json",
  );
  ...
  if (optimizedPackage?.package_boundary_audit) {
    writeJsonFile(
      packageBoundaryAuditPath,
      optimizedPackage.package_boundary_audit,
    );
  }
  ...
  timings.manifest_checksum = durationSeconds(
    manifestChecksumStartedAt,
    monotonicSeconds(),
  );
    ...
    JSON.stringify(
      {
        ...
        runtime_cache_events: runtimeCacheEventsPath,
        runtime_native_trust: runtimeNativeTrustPath,
        app_bundle_trim_report: optimizedPackage?.app_bundle_trim
          ? appBundleTrimPath
          : null,
        package_boundary_audit: optimizedPackage?.package_boundary_audit
          ? packageBoundaryAuditPath
          : null,
        precompression_gate: precompressionGatePath,
        timing: timingPath,
