...
export function commandOutput(
  ...
  return String(
    commandResult(command, args, { ...options, capture: true }).stdout ?? "",
  ).trim();
}
...
export function deriveManualLocalAppIdentity(
  ...
  if (!/^\d+\.\d+\.\d+$/.test(publicUpdaterVersion)) {
    throw new Error(
      `Manual local App public updater version is invalid: ${publicUpdaterVersion || "<empty>"}`,
    );
  }
  if (!/^[0-9a-f]{64}$/.test(sourceProvenanceSha256)) {
    throw new Error(
      "Manual local App source provenance SHA-256 must be an exact lowercase hex digest",
    );
  }
  return {
    build_kind: "local-development",
...
function setPlistString(plistPath: string, key: string, value: string) {
  ...
  if (observed !== value) {
    throw new Error(
      `Manual local App Info.plist ${key} mismatch after stamping`,
    );
  }
}
...
export function stampManualLocalAppIdentity(
  ...
  if (
    ...
    throw new Error(
      "Manual local App identity does not match its source provenance",
    );
  }
  ...
  const launchEnvironment = commandResult(
    "plutil",
  ...
  setPlistString(plistPath, "LSEnvironment.AIONUI_DISABLE_AUTO_UPDATE", "1");
  return identity;
}
...
export function githubApi<T = JsonRecord>(apiPath: string): T {
  ...
  const headers = [
    ...
    "-H",
    "User-Agent: one-person-lab-manual-latest-build",
  ];
...
export function extractVerifiedTarGz(archivePath: string, targetRoot: string) {
  ...
  commandResult("tar", ["-xzf", archivePath, "-C", targetRoot], {
    timeoutMs: 120_000,
  });
}

export function verifyMacArm64Binary(
  binaryPath: string,
  versionArgs: string[],
  label: string,
) {
  ...
  const versionOutput = commandOutput(binaryPath, versionArgs, {
    timeoutMs: 30_000,
  });
  if (process.platform === "darwin") {
    commandResult("codesign", ["--verify", "--verbose=2", binaryPath], {
      timeoutMs: 30_000,
    });
  }
...
export function manualVersions(now = new Date()) {
  ...
  return {
    displayVersion,
    updaterVersion: resolveReleaseVersionIdentity("stable", displayVersion)
      .updaterVersion,
  };
}
...
export function snapshotDevelopmentRepo(
  id: string,
  root: string,
): RepoSnapshot {
  ...
  const localMainResult = commandResult(
    "git",
  ...
  const localMain =
    localMainResult.status === 0 ? String(localMainResult.stdout).trim() : null;
  ...
  if (originUrl.status === 0 && String(originUrl.stdout).trim()) {
    ...
    const match =
      remote.status === 0
        ? /^([0-9a-f]{40})\s+refs\/heads\/main$/m.exec(
...
export function assertDevelopmentRepoSnapshotsUnchanged(
