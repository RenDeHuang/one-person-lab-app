...
export const releaseBoundaryChecks: ReleaseBoundaryCheck[] = [
  ...
  },
  {
    ...
    required: [
      "release_set_generation=$OPL_RELEASE_SET_GENERATION",
      "release_gate=$OPL_RELEASE_GATE",
      "promotion_target=$target",
      "promotion_request_id=$OPL_PROMOTION_REQUEST_ID",
      "source_app_run_id=$OPL_SOURCE_APP_RUN_ID",
      "expected_app_source_commit=$OPL_APP_SOURCE_COMMIT",
      "expected_app_artifact_digest=$OPL_APP_ARTIFACT_DIGEST",
      "expected_framework_source_commit=$OPL_FRAMEWORK_SOURCE_COMMIT",
      "expected_carrier_digest=$OPL_EXPECTED_CARRIER_DIGEST",
  ...
  },
  {
    ...
    required: [
      ...
      "OPL_RELEASE_NOTES_MODEL: ${{ vars.OPL_RELEASE_NOTES_MODEL || 'gpt-5.6-luna' }}",
  ...
  },
  {
    ...
    required: [
      ...
      "webui-ghcr-publish:",
      "base_version=\"$(date -u +'%y.%-m.%-d')-nightly\"",
      'existing_refs_file="$RUNNER_TEMP/opl-nightly-existing-refs.txt"',
      "timeout 30s git ls-remote --tags origin",
      "timeout 30s gh release list",
      "npm run --silent release:nightly-version:resolve --",
    ...
    forbidden: [
      ...
      "--force-with-lease=",
      "--clobber",
      "nightly.${GITHUB_RUN_ID}.${GITHUB_RUN_ATTEMPT}",
    ],
  },
  {
  ...
  },
  {
    ...
    required: [
      ...
      "blocked",
      "diagnostic_only",
      "Only ready_to_promote candidate records may authorize the receipt-gated desktop-release-promote saga.",
      "release:stable -- promote",
    ...
    forbidden: ["TODO", "TBD", "gh release edit"],
  },
  {
  ...
  },
  {
    ...
    required: [
      ...
      '--field standard_vm_evidence_base64="$STANDARD_VM_EVIDENCE_BASE64"',
      ...
      'release_set_generation="${{ inputs.release_set_generation }}"',
      'release_set_manifest_digest="${{ needs.framework-release-set.outputs.carrier_digest }}"',
      ...
      "Queue asynchronous same-cohort Full add-on",
  ...
  {
    id: "desktop_release_full_addon_is_additive_only",
    file: ".github/workflows/desktop-release-full-addon.yml",
    required: [
      "name: OPL Desktop Full Add-on",
      "Verify published Standard and frozen add-on cohort",
      "test \"$GITHUB_SHA\" = \"${{ inputs.app_sha }}\"",
      "Bind add-on inputs to the immutable Standard distribution receipt",
      "opl-stable-distribution-receipt-${{ inputs.opl_version }}-${session_hex}",
      "scripts/validate-release-saga-receipt.ts",
      "uses: ./.github/workflows/full-first-install-release.yml",
      "publish_to_release: false",
      "release_artifact_run_id: ${{ github.run_id }}",
      "package_profile: full",
      "Validate exact build and qualification cohort",
      "scripts/validate-build-artifact-cohort.ts",
      "scripts/validate-artifact-qualification-receipt.ts",
      "Attest Full add-on assets",
    ...
    forbidden: [
      ...
      "latest-mac.yml",
      "oras tag",
      "stable-standard-distribution.yml",
      "framework-release-promotion-step.sh",
    ],
  },
  {
