...
test('same-run Native manifests admit only their exact closed cross-platform asset union', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'opl-native-manifest-release-receipt-'));
  const bundlePath = path.join(root, 'bundle.json');
  const inspectionPath = path.join(root, 'remote-before.json');
  const linuxManifestPath = path.join(root, 'linux-manifest.json');
  const darwinManifestPath = path.join(root, 'darwin-manifest.json');
  const requiredNames = ['first.zip', 'second.dmg'];
  const nativeManifest = (platform: 'linux' | 'darwin', architecture: 'x86_64' | 'arm64') => {
    const base = `one-person-lab-webui-${tag.slice(1)}-${platform}-${architecture}`;
    const identities = [
      ['runtime_tarball', `${base}.tar.gz`, platform === 'linux' ? '7' : '8'],
      ['runtime_metadata', `${base}.tar.gz.sha256`, platform === 'linux' ? '9' : 'a'],
      ['installer', 'install-web.sh', 'b'],
      ['installer_sha256', 'install-web.sh.sha256', 'c'],
      ['qualification_receipt', `${base}.qualification.json`, platform === 'linux' ? 'd' : 'e'],
    ];
    return {
      ...
      platform,
      architecture,
      release_bundle_digest: bundleDigest,
      ...
      qualification_receipt: { path: `${base}.qualification.json`, sha256: 'f'.repeat(64) },
      assets: identities.map(([role, name, byte]) => ({
        role,
        name,
        path: name,
        size_bytes: 100,
  ...
  const linuxManifest = nativeManifest('linux', 'x86_64');
  const darwinManifest = nativeManifest('darwin', 'arm64');
  const remoteAsset = (entry: Record<string, any>) => ({
    name: entry.name,
  ...
  const execute = (
    ...
    manifests = [linuxManifestPath, darwinManifestPath],
    ...
    return buildExecutorReceipt({
      operation: 'remote_inspect',
      'release-operation': 'resume_standard',
      'operation-id': standardOperationId,
      executor: 'remote',
      'attempt-id': workflowAttemptId,
      'remote-target': `github-release:${repo}@${tag}`,
      ...
      bundle: bundlePath,
      inspection: inspectionPath,
      'allow-same-tag-full-assets': 'true',
      'allow-exact-native-manifest': manifests,
      'require-exact-native-manifest': requiredManifests,
      'native-stable-authority-run-id': '12345',
    } as any);
  };
  try {
    fs.writeFileSync(bundlePath, `${JSON.stringify({
    ...
    fs.writeFileSync(linuxManifestPath, `${JSON.stringify(linuxManifest)}\n`);
    fs.writeFileSync(darwinManifestPath, `${JSON.stringify(darwinManifest)}\n`);
    const second = asset(requiredNames[1]!, '2');
    const union = [
      second,
      asset('full.dmg', '6'),
      ...new Map(
        [...linuxManifest.assets, ...darwinManifest.assets]
          .map(remoteAsset)
          .map((entry) => [entry.name, entry]),
      ).values(),
    ];
    assert.deepEqual(execute(union).assets, [{
      name: second.name,
      size_bytes: second.size_bytes,
      sha256: second.sha256,
    }]);
    assert.throws(
      () => execute(union.map((entry) => entry.name === 'install-web.sh'
        ? { ...entry, sha256: `sha256:${'0'.repeat(64)}` }
        : entry)),
      /Remote Native WebUI asset install-web\.sh differs from its exact admitted manifest/,
    );
    assert.throws(
      () => execute([...union, asset('unknown-native.bin', '1')]),
      /contains unknown asset unknown-native\.bin/,
    );
    assert.throws(
      () => execute(
        union.filter((entry) => entry.name !== linuxManifest.assets[0]!.name),
        [darwinManifestPath],
        [linuxManifestPath],
      ),
      /required by prior exact manifest is missing/,
    );
    assert.throws(
      () => execute(union, [linuxManifestPath, linuxManifestPath]),
      /must describe unique supported targets/,
    );
    assert.throws(
      () => execute(union, [linuxManifestPath, darwinManifestPath, linuxManifestPath]),
      /accepts at most two exact Native WebUI manifests/,
    );
    assert.throws(
      () => buildExecutorReceipt({
        operation: 'remote_inspect',
        ...
        inspection: inspectionPath,
        'allow-same-tag-full-assets': 'true',
        'allow-exact-native-manifest': [linuxManifestPath, darwinManifestPath],
        'native-stable-authority-run-id': '54321',
      } as any),
      /does not match the immutable Stable Bundle authority/,
    );
    const conflictingDarwin = structuredClone(darwinManifest);
    const conflictingInstaller = conflictingDarwin.assets.find((entry) => entry.name === 'install-web.sh');
    assert.ok(conflictingInstaller);
    conflictingInstaller.sha256 = '0'.repeat(64);
    fs.writeFileSync(darwinManifestPath, `${JSON.stringify(conflictingDarwin)}\n`);
    assert.throws(
      () => execute(union),
      /manifests disagree on shared asset install-web\.sh/,
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
