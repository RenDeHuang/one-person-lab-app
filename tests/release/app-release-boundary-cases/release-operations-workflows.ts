...
test('Nightly release workflow publishes standard-only semver prereleases', () => {
  ...
  assert.match(workflow, /prepare-nightly-release-notes:/);
  assert.match(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /OPL_RELEASE_NOTES_MODE: ai/);
  assert.match(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /OPL_RELEASE_NOTES_PROVIDER: openai_compatible/);
  assert.match(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /OPL_RELEASE_NOTES_CODEX_BASE_URL: \$\{\{ vars\.OPL_RELEASE_NOTES_CODEX_BASE_URL \|\| 'https:\/\/gflabtoken\.cn\/v1' \}\}/);
  assert.match(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /OPL_RELEASE_NOTES_MODEL: \$\{\{ vars\.OPL_RELEASE_NOTES_MODEL \|\| 'gpt-5\.4-mini' \}\}/);
  assert.match(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /npm run release:notes:probe-ai/);
  assert.match(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /node --experimental-strip-types scripts\/generate-release-notes\.ts[\s\S]*--channel nightly/);
  assert.match(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /OPL_RELEASE_NOTES_EVIDENCE_OUTPUT: \$\{\{ runner\.temp \}\}\/opl-nightly-notes-evidence\.json/);
  assert.match(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /node --experimental-strip-types scripts\/generate-release-notes\.ts[\s\S]*--evidence-output "\$OPL_RELEASE_NOTES_EVIDENCE_OUTPUT"[\s\S]*--output "\$template_notes_file"/);
  assert.match(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /node --experimental-strip-types scripts\/release-notes-ai-writer\.ts[\s\S]*--evidence "\$OPL_RELEASE_NOTES_EVIDENCE_OUTPUT"[\s\S]*--output "\$notes_file"/);
  assert.doesNotMatch(workflowJobBlock(workflow, 'prepare-nightly-release-notes'), /--input "\$template_notes_file"/);
  assert.match(workflow, /prepared-nightly-release-notes-\$\{\{ needs\.resolve-nightly\.outputs\.version \}\}/);
  assert.match(workflowJobBlock(workflow, 'publish-nightly'), /Download prepared Nightly release notes/);
  assert.match(workflowJobBlock(workflow, 'publish-nightly'), /notes_file="prepared-nightly-release-notes\/opl-nightly-notes\.md"/);
  assert.doesNotMatch(workflowJobBlock(workflow, 'publish-nightly'), /release-notes-ai-writer/);
  assert.doesNotMatch(workflowJobBlock(workflow, 'publish-nightly'), /OPL_RELEASE_NOTES_OPENAI_COMPATIBLE_API_KEY/);
  assert.doesNotMatch(workflowJobBlock(workflow, 'publish-nightly'), /OPL_RELEASE_NOTES_CODEX_API_KEY/);
  assert.match(workflow, /\$\{\{ runner\.temp \}\}\/opl-nightly-notes\.md/);
  assert.match(workflow, /\$\{\{ runner\.temp \}\}\/opl-nightly-template-notes\.md/);
...
test('release workflows resolve moving refs once and pass fixed SHA cohort refs downstream', () => {
  ...
  assert.match(workflowJobBlock(desktopWorkflow, 'prepare-standard-release-notes'), /OPL_RELEASE_NOTES_MODE: ai/);
  ...
  assert.match(workflowJobBlock(desktopWorkflow, 'prepare-standard-release-notes'), /OPL_RELEASE_NOTES_CODEX_BASE_URL: \$\{\{ vars\.OPL_RELEASE_NOTES_CODEX_BASE_URL \|\| 'https:\/\/gflabtoken\.cn\/v1' \}\}/);
  assert.match(workflowJobBlock(desktopWorkflow, 'prepare-standard-release-notes'), /OPL_RELEASE_NOTES_MODEL: \$\{\{ vars\.OPL_RELEASE_NOTES_MODEL \|\| 'gpt-5\.4-mini' \}\}/);
  assert.match(workflowJobBlock(desktopWorkflow, 'prepare-standard-release-notes'), /npm run release:notes:probe-ai/);
  ...
  assert.doesNotMatch(workflowJobBlock(desktopWorkflow, 'prepare-standard-release-notes'), /--input standard-release-notes-template\.md/);
  ...
  assert.match(workflowJobBlock(desktopWorkflow, 'prepare-full-release-notes'), /OPL_RELEASE_NOTES_MODE: ai/);
  assert.match(workflowJobBlock(desktopWorkflow, 'prepare-full-release-notes'), /OPL_RELEASE_NOTES_PROVIDER: openai_compatible/);
  assert.match(workflowJobBlock(desktopWorkflow, 'prepare-full-release-notes'), /OPL_RELEASE_NOTES_CODEX_BASE_URL: \$\{\{ vars\.OPL_RELEASE_NOTES_CODEX_BASE_URL \|\| 'https:\/\/gflabtoken\.cn\/v1' \}\}/);
  assert.match(workflowJobBlock(desktopWorkflow, 'prepare-full-release-notes'), /OPL_RELEASE_NOTES_MODEL: \$\{\{ vars\.OPL_RELEASE_NOTES_MODEL \|\| 'gpt-5\.4-mini' \}\}/);
  assert.match(workflowJobBlock(desktopWorkflow, 'prepare-full-release-notes'), /npm run release:notes:probe-ai/);
  ...
  assert.doesNotMatch(workflowJobBlock(desktopWorkflow, 'prepare-full-release-notes'), /--input full-release-notes-template\.md/);
