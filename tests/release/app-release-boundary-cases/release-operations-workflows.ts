...
test('Nightly release workflow publishes standard-only semver prereleases', () => {
  ...
  assert.doesNotMatch(workflow, /OPL_RELEASE_NOTES_CODEX_API_KEY/);
  ...
  assert.match(workflow, /node --experimental-strip-types scripts\/release-notes-ai-writer\.ts[\s\S]*--evidence "\$OPL_RELEASE_NOTES_EVIDENCE_OUTPUT"[\s\S]*--input "\$template_notes_file"[\s\S]*--output "\$notes_file"/);
