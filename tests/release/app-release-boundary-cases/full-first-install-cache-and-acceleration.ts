...
test('Full first-install cache and release acceleration contract are explicit', async () => {
  ...
  assert.match(buildScript, /npm_bin_sha256: existingFileSha256\(sources\.nodeToolchain\.npmBin\)/);
  assert.match(buildScript, /npx_bin_sha256: existingFileSha256\(sources\.nodeToolchain\.npxBin\)/);
  ...
  assert.match(buildScript, /bun_sha256: sources\.bunBin \? existingFileSha256\(sources\.bunBin\) : null/);
  assert.match(buildScript, /temporal_cli_sha256: existingFileSha256\(sources\.temporalCliBin\)/);
  assert.match(buildScript, /temporal_cli_version: commandOutput\(sources\.temporalCliBin, \['--version'\]\)/);
  assert.match(buildScript, /temporal_cli_archive_sha256: existingFileSha256\(sources\.temporalCliArchive\)/);
