...
import { selectLatestMineruCliRelease } from "../../../scripts/manual-latest-build/upstreams.ts";

const appRoot = path.resolve(import.meta.dirname, "..", "..", "..");

function createDevelopmentRepo() {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "opl-manual-source-snapshot-"),
  );
  execFileSync("git", ["init", "--initial-branch=main"], { cwd: root });
  execFileSync("git", ["config", "user.name", "OPL Test"], { cwd: root });
  execFileSync("git", ["config", "user.email", "opl-test@example.invalid"], {
    cwd: root,
  });
...
`,
  ...
  if (input.marker)
    fs.writeFileSync(path.join(contents, input.marker), "\n", "utf8");
}
...
`,
  ...
  writeExecutable(
    path.join(binaryRoot, "ditto"),
    `#!/bin/sh
exec /bin/cp -R "$1" "$2"
`,
  );
  writeExecutable(
    path.join(binaryRoot, "pgrep"),
    `#!/bin/sh
...
`,
  ...
  writeExecutable(
    path.join(binaryRoot, "osascript"),
    `#!/bin/sh
echo 0 > "$OPL_TEST_RUNNING_STATE"
`,
  );
  writeExecutable(
    path.join(binaryRoot, "open"),
    `#!/bin/sh
...
test("manual local App identity is deterministic without corrupting the updater machine version", () => {
  ...
  const identity = deriveManualLocalAppIdentity(
    "26.7.2803",
...
test(
  "manual local App plist stamping exposes public updater and source-lock provenance",
  {
    skip: process.platform !== "darwin",
  },
  (context) => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), "opl-manual-local-identity-"),
    );
...
test("full package app-only build fails closed without manual local identity", () => {
  const script = path.join(
    appRoot,
    "scripts",
    "build-full-first-install-package.ts",
  );
...
test(
  "manual installer replaces a runtime-mutated baseline and types a failed replacement rollback",
  {
    skip: process.platform !== "darwin",
  },
  () => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), "opl-manual-install-test-"),
    );
    ...
    try {
      ...
      const successInstall = path.join(
        successRoot,
        "Applications",
        "One Person Lab.app",
      );
      const successBuilt = path.join(
        successRoot,
        "built",
        "One Person Lab.app",
      );
      ...
      createTestApp(successBuilt, {
        ...
        buildIdentity: {
          ...
          sourceProvenanceSha256:
            expectedVersionIdentity.source_provenance_sha256,
      ...
      assert.match(
        completed.replaced_signature?.diagnostics ?? "",
        /sealed resource/,
      );
      ...
      assert.equal(
        fs.existsSync(
          path.join(successInstall, "Contents", "OLD_SIGNATURE_POLLUTION"),
        ),
        false,
      );

      const failureRoot = path.join(root, "failure");
      const failureInstall = path.join(
        failureRoot,
        "Applications",
        "One Person Lab.app",
      );
      const failureBuilt = path.join(
        failureRoot,
        "built",
        "One Person Lab.app",
      );
      ...
      assert.equal(
        failure.receipt.rollback.baseline_preserved_at_install_path,
        true,
      );
      ...
      assert.equal(
        fs.existsSync(
          path.join(failureInstall, "Contents", "OLD_SIGNATURE_POLLUTION"),
        ),
        true,
      );
      assert.equal(
        fs.existsSync(
          path.join(failureInstall, "Contents", "FAIL_FINAL_SIGNATURE"),
        ),
        false,
      );
      assert.deepEqual(
        fs
          .readdirSync(path.dirname(failureInstall))
          .filter((entry) => entry.startsWith(".opl-manual-app-")),
        [],
      );
    } finally {
      if (originalPath === undefined) delete process.env.PATH;
      else process.env.PATH = originalPath;
      if (originalInstallPath === undefined)
        delete process.env.OPL_TEST_INSTALL_PATH;
      else process.env.OPL_TEST_INSTALL_PATH = originalInstallPath;
      if (originalRunningState === undefined)
        delete process.env.OPL_TEST_RUNNING_STATE;
...
test("manual source snapshot rejects a clean local main behind fetched origin/main", (context) => {
  ...
  execFileSync("git", ["update-ref", "refs/remotes/origin/main", remoteHead], {
    cwd: root,
  });
...
test("manual source snapshot rejects a stale tracking ref after remote main advances", (context) => {
  ...
  const remoteRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "opl-manual-source-remote-"),
  );
  context.after(() => {
    fs.rmSync(root, { recursive: true, force: true });
  ...
  execFileSync("git", ["config", "user.name", "OPL Publisher"], {
    cwd: publisher,
  });
  execFileSync("git", ["config", "user.email", "publisher@example.invalid"], {
    cwd: publisher,
  });
...
test("manual latest build resolves UI UX Pro Max only from its owner checkout", () => {
  ...
  assert.doesNotMatch(
    source,
    new RegExp(["ai", "skills", "library"].join("-")),
  );
});
