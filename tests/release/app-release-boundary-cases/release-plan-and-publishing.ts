...
test('existing same-tag standard plus Full publish uses deterministic full release notes body', () => {
  ...
  assert.match(fullWorkflow, /Prepare Full release notes[\s\S]*OPL_RELEASE_NOTES_MODE: template/);
  assert.match(fullWorkflow, /Prepare Full release notes[\s\S]*--input "\$RUNNER_TEMP\/full-release-notes-template\.md"/);
  assert.doesNotMatch(fullWorkflow, /Prepare Full release notes[\s\S]*OPL_RELEASE_NOTES_OPENAI_COMPATIBLE_API_KEY/);
  assert.doesNotMatch(fullWorkflow, /Prepare Full release notes[\s\S]*OPL_RELEASE_NOTES_CODEX_API_KEY/);
  ...
  assert.doesNotMatch(fullWorkflow, /node --experimental-strip-types scripts\/release-notes-ai-writer\.ts --probe-openai-compatible/);
