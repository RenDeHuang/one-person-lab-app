...
test('existing same-tag standard plus Full publish uses deterministic full release notes body', () => {
  ...
  assert.match(fullWorkflow, /Publish GitHub Release assets[\s\S]*OPL_RELEASE_NOTES_CODEX_BASE_URL: \$\{\{ vars\.OPL_RELEASE_NOTES_CODEX_BASE_URL \}\}/);
  assert.match(fullWorkflow, /Publish GitHub Release assets[\s\S]*OPL_RELEASE_NOTES_CODEX_API_KEY: \$\{\{ secrets\.OPL_RELEASE_NOTES_CODEX_API_KEY \}\}/);
