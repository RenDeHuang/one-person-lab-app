...
test('existing same-tag standard plus Full publish uses deterministic full release notes body', () => {
  ...
  assert.match(fullWorkflow, /Prepare Full release notes[\s\S]*OPL_RELEASE_NOTES_OPENAI_COMPATIBLE_BASE_URL: \$\{\{ vars\.OPL_RELEASE_NOTES_OPENAI_COMPATIBLE_BASE_URL \}\}/);
  assert.match(fullWorkflow, /Prepare Full release notes[\s\S]*OPL_RELEASE_NOTES_OPENAI_COMPATIBLE_API_KEY: \$\{\{ secrets\.OPL_RELEASE_NOTES_OPENAI_COMPATIBLE_API_KEY \}\}/);
  assert.match(fullWorkflow, /Prepare Full release notes[\s\S]*OPL_RELEASE_NOTES_CODEX_BASE_URL: \$\{\{ vars\.OPL_RELEASE_NOTES_CODEX_BASE_URL \}\}/);
  assert.match(fullWorkflow, /Prepare Full release notes[\s\S]*OPL_RELEASE_NOTES_CODEX_API_KEY: \$\{\{ secrets\.OPL_RELEASE_NOTES_CODEX_API_KEY \}\}/);
