...
test('manual desktop release workflow supports new releases and same-tag refreshes in GitHub Actions', () => {
  ...
  assert.match(workflow, /full-first-install:[\s\S]*needs:[\s\S]*release-workflow-contract[\s\S]*release-source-gate/);
  ...
  assert.match(workflow, /uses: \.\/\.github\/workflows\/full-first-install-release\.yml[\s\S]*framework_ref: \$\{\{ needs\.release-source-gate\.outputs\.framework_sha \}\}[\s\S]*shell_ref: \$\{\{ needs\.release-source-gate\.outputs\.shell_sha \}\}/);
  ...
  assert.match(workflow, /publish-full-assets:[\s\S]*needs:[\s\S]*standard-vm-smoke-gate-after-full/);
  assert.match(workflow, /publish-full-assets:[\s\S]*needs\.standard-vm-smoke-gate-after-full\.result == 'success'/);
  ...
  assert.match(workflow, /Standard VM smoke must pass before Full publish, remote verification, Homebrew, operator evidence, or readiness aggregation can run/);
  ...
  assert.match(workflow, /subject-path: full-package-artifacts\/\*/);
  assert.match(workflow, /subject-digest: \$\{\{ steps\.build_docker_webui\.outputs\.full_digest \}\}/);
  assert.match(workflow, /subject-digest: \$\{\{ steps\.build_docker_webui\.outputs\.slim_digest \}\}/);
  assert.match(dockerWebuiJob, /create-storage-record: false/);
  assert.match(dockerWebuiJob, /permissions:[\s\S]*packages: write[\s\S]*attestations: write[\s\S]*id-token: write/);
  ...
  assert.match(dockerWebuiCleanVmEvidenceJob, /windows_zip_evidence=""[\s\S]*if \[ -d "\$windows_raw_evidence" \]; then[\s\S]*find "\$windows_raw_evidence" -maxdepth 1 -type f -name '\*\.zip'/);
  ...
  assert.equal((workflow.match(/^\s+docker build \\/gm) ?? []).length, 2);
