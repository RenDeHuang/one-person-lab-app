...
test('manual desktop release workflow supports new releases and same-tag refreshes in GitHub Actions', () => {
  ...
  assert.match(workflow, /OPL_RELEASE_NOTES_CODEX_BASE_URL: \$\{\{ vars\.OPL_RELEASE_NOTES_CODEX_BASE_URL \|\| 'https:\/\/gflabtoken\.cn\/v1' \}\}/);
  assert.match(workflow, /OPL_RELEASE_NOTES_MODEL: \$\{\{ vars\.OPL_RELEASE_NOTES_MODEL \|\| 'gpt-5\.4-mini' \}\}/);
  assert.match(workflow, /npm run release:notes:probe-ai/);
  ...
  assert.doesNotMatch(workflow, /--input standard-release-notes-template\.md/);
  assert.doesNotMatch(workflow, /--input full-release-notes-template\.md/);
