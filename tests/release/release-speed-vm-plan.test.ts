...
test('desktop release workflow keeps the release DAG split by build, publish, verification, and VM gates', () => {
  ...
  assertMatches(workflow, /homebrew-standard-first-run-vm-smoke:[\s\S]*?needs:\s+stable-homebrew-tap-update/, 'Homebrew VM smoke job');
  ...
  assertMatches(workflow, /release-readiness-summary:[\s\S]*?remote-verify-standard[\s\S]*?remote-verify-full[\s\S]*?standard-first-run-vm-smoke-after-standard-only[\s\S]*?standard-first-run-vm-smoke-after-full[\s\S]*?stable-homebrew-tap-update[\s\S]*?homebrew-standard-first-run-vm-smoke[\s\S]*?full-first-run-vm-smoke[\s\S]*?one-shot-app-installer-smoke[\s\S]*?docker-webui-smoke/, 'final release readiness dependencies');
