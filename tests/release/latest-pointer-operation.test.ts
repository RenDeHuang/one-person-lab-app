...
function fixture(kind: 'dev' | 'nightly' = 'dev') {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'opl-latest-pointer-'));
  const version = kind === 'dev' ? '26.7.27-preview.r1' : '26.7.27-nightly.r1';
  const updaterVersion = kind === 'dev' ? '26.7.2701' : '26.7.2791-nightly.1';
  const tag = `v${version}`;
  const assetNames = [
...
test('published Dev and Nightly Preview identities admit an exact quality-preserving Latest operation', () => {
  for (const kind of ['dev', 'nightly'] as const) {
    ...
    try {
      ...
      assert.equal(current.receipt.candidate.quality_status, 'preview');
      assert.equal(current.receipt.candidate.preview_kind, kind);
      ...
      assert.equal(current.receipt.public_assets.length, kind === 'dev' ? 7 : 5);
    } finally {
