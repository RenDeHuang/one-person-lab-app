...
test('new Standard consumes frozen protected evidence before sealing its run-bound control', () => {
  ...
  assert.match(
    stableAdmission,
    /test -z "\$REQUESTED_VERSION\$REQUESTED_SHELL_REF\$REQUESTED_SMOKE_HARNESS_REF\$REQUESTED_FRAMEWORK_REF\$SOURCE_RUN_ID\$SOURCE_ARTIFACT\$PRIOR_FULL_ARTIFACT_RUN_ID"/,
  );
